# Exercise 6 講義: VLAN の基礎

> 配置先: ドキュメント `01_教材 > LESSON2 > Exercise6`
...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 5「ルーティングの基礎と静的ルート」 / Exercise 3「Cisco IOS の基本操作とデバイス初期設定」 / Exercise 1「ネットワークの全体像と OSI / TCP-IP モデル」 の範囲から出題）。

**W1.** `show ip route` に表示されるルートコード `C`・`L`・`S`・`S*` は、
それぞれ何を表すか。

**W2.** running-config を、再起動しても消えない startup-config（NVRAM）に保存する
コマンドは何か。

**W3.** OSI 参照モデルの第 1 層（物理層）が扱うものを一言で答えよ。

<details><summary>解答</summary>

W1. C = 直接接続（Connected）／L = インターフェース自身の /32（Local）／
S = 静的ルート（Static）／S* = デフォルトルート
W2. `copy running-config startup-config`（`write memory` でも可）
W3. ビット（電気信号・光信号・電波としての 0/1 の伝送）
...
- **異なる VLAN に属する端末同士は、L2 だけでは通信できません**。通信するには、
  ...
  （VLAN 間ルーティングの詳細は Exercise 8 で扱います）。

![VLAN の有無でブロードキャストドメインの範囲が変わる](../images/exercise06-vlan-broadcast-domain.png)

### VLAN と IP サブネットの対応

VLAN 設計の基本原則として、**1 つの VLAN は原則 1 つの IP サブネットに対応**させます。
たとえば VLAN 10 に `192.168.10.0/24` を割り当てる、といった具合です。この結果、
アクセス層スイッチ（PC などの端末が直接つながる末端のスイッチ）上で
「ブロードキャストドメイン = VLAN = IP サブネット」という
...
| **ネイティブ VLAN**※ | トランクリンク上で、タグを付けずに送受信する VLAN。既定値は VLAN 1（詳細は Exercise 7 のトランクで扱う） |

> ※ ネイティブ VLAN は、上記 3 つのような端末を収容する「用途区分」ではなく、
> トランクリンク上の 1 プロパティ（タグを付けずに扱う VLAN）です。既定値が
> デフォルト VLAN と同じ VLAN 1 であるため混同しやすいですが、デフォルト VLAN
> （初期状態で全アクセスポートが所属する VLAN 1）とは別の概念である点に注意してください。
...
| 　うち 1002〜1005 | （標準範囲内の）予約 VLAN | FDDI / トークンリング用に予約済み（通常は使用しない） |
| 1006〜4094 | 拡張範囲 VLAN | 大規模環境向け。スイッチの動作モードによっては制限あり |

> 1002〜1005 は独立した別範囲ではなく、標準範囲（1〜1005）に含まれる一部の
> VLAN 番号です。「標準範囲・予約範囲・拡張範囲」の 3 つが互いに重ならず並んでいる
> わけではない点に注意してください。
...
> 💼 **実務では**: 保守現場では `show vlan brief` や `show interfaces trunk` を、
> 「VLAN 1 に業務トラフィックや管理用 SVI が乗っていないか」を確認する監視・
> 切り分けの道具として使います。ただし異常に気づいても、客先の設備を自己判断で
> 変更してはいけません。変更は手順書と作業依頼の範囲内でのみ行うのが原則で、
> 気づいた点はまず先輩や元請けへ報告・エスカレーションし、対応方針の指示を待ちます。
...
> あります。ただし `delete flash:vlan.dat` や `erase startup-config` のような
> 機器全体に影響するコマンドは、手順書に明記された作業（機器の転用・返却など）
> 以外では絶対に自己判断で実行しません。想定外の挙動を見つけたときは、その場で
> 対処せず作業手順書との食い違いをまず確認し、判断に迷えば先輩へエスカレーションします。
...
L3 機器によるルーティングを別途構成する必要があります（Exercise 8）。
...
この Exercise のラボ課題「[Exercise6] ラボ: 1 台のスイッチに 2 つの VLAN を構成し、

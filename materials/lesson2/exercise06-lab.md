...
![Exercise 6 トポロジ図](../images/exercise06-topology.png)
...
> 設定不要です（Exercise 8 で VLAN 間ルーティングを学んだ後に再度使用します）。
...
7. `File > Save As` でファイルを保存する: `exercise06_氏名.pkt`
...
3. PC1 の Command Prompt から、異なる VLAN の PC3 へ ping する
   ...
   **確認**: `Reply from 192.168.10.11: Destination host unreachable.` と表示され、
   異なる VLAN 間では疎通しないことを確認する（宛先 192.168.20.21 は PC1 と別サブネットであり、
   かつデフォルトゲートウェイが未設定のため、PC は送信前にルーティング不能と判断し、
   自分自身のアドレスから到達不能の応答を返します。今回の構成では異なる VLAN 間は
   ブロードキャストドメインだけでなく IP サブネットも分かれているため、L3 機器と
   デフォルトゲートウェイの設定がなければ通信できません）
...
1. PC1→PC2 は成功し PC1→PC3 は失敗しました（PC1 の画面には
   `Destination host unreachable.` と表示されたはずです）。この差が生まれる理由を、
   「ブロードキャストドメイン」と「デフォルトゲートウェイ」の両方の語を使って
   説明してください。
...
1. `exercise06_氏名.pkt` を Backlog のラボ課題に**添付**する

...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 7「トランクと VLAN 設計」 / Exercise 5「ルーティングの基礎と静的ルート」 / Exercise 3「Cisco IOS の基本操作とデバイス初期設定」 の範囲から出題）。

**W1.** トランクポートでフレームに VLAN 情報を付ける仕組みの標準規格名と、
タグの大きさを答えよ。

**W2.** ルータが経路を選ぶときの 3 段階の判断順序を答えよ。

**W3.** スイッチやルータで SSH を有効化するために必要な設定を 4 つ挙げよ。

<details><summary>解答</summary>

W1. IEEE 802.1Q／4 バイトのタグをイーサネットフレームに挿入する
W2. ロンゲストマッチ（最長プレフィックス一致）→ AD（管理距離）→ メトリック
W3. ホスト名の設定／`ip domain-name` の設定／`crypto key generate rsa` による鍵生成／
line vty での `login local` と `transport input ssh`（＋ユーザ名の作成）
...
Exercise 6・7 で学んだ VLAN（Virtual LAN、仮想 LAN）は、1 台の物理スイッチを複数の
...
各 VLAN が別サブネットである以上、VLAN ごとに**デフォルトゲートウェイ**（LESSON0 P5
...
あります（Exercise 7 参照）。ネイティブ VLAN 用のサブインターフェースを設定する場合は、
...
![SVIがup/upになるための3条件（AND条件）](../images/exercise08-svi-conditions.png)
...
L3 スイッチの VLAN 間転送は、LESSON0 P1 で学んだ **CPU** によるソフトウェア処理では

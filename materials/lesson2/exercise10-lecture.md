# Exercise 10 講義: 無線 LAN と検出プロトコル

> 配置先: ドキュメント `01_教材 > LESSON2 > Exercise10`
...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 9「STP と EtherChannel」 / Exercise 7「トランクと VLAN 設計」 / Exercise 5「ルーティングの基礎と静的ルート」 の範囲から出題）。

**W1.** STP でルートブリッジに選ばれるのは、BID がどうなっているスイッチか。
BID の構成要素も答えよ。

**W2.** ネイティブ VLAN のフレームは、トランク上でどう扱われるか。

**W3.** 直接接続・静的ルート・OSPF の AD（管理距離）の既定値をそれぞれ答えよ。

<details><summary>解答</summary>

W1. BID が最小のスイッチ。BID = ブリッジプライオリティ（既定 32768）＋ MAC アドレス
W2. タグを付けずに（untagged で）送信される
W3. 直接接続 = 0／静的ルート = 1／OSPF = 110
...
CSMA/CA は「送信前にチャネルが空いているかを電波で確認する」（キャリアセンス）
ことを前提にしています。しかしこの前提が崩れる状況があります。
ここで **STA** とは Station の略で、AP に接続する PC やスマートフォンなどの
無線クライアント端末を指します。いま 2 台の STA があり、どちらも同じ AP の
電波は届くとします。ところがこの 2 台は**互いの電波が届かない位置関係**に
あるとします。たとえば AP を挟んで反対側にいて、間に壁がある場合です。
このとき各 STA は相手の送信を電波として検知できないため、
チャネルが空いていると誤って判断し、**同時に送信してしまう**ことがあります。
その結果、両者の電波が届く AP 側で衝突が発生しやすくなります。これを
**隠れ端末問題**（hidden node problem：互いに「隠れている」端末による問題）と
呼びます。

この問題を緩和するのが **RTS/CTS**（Request to Send／Clear to Send：送信要求／
送信可）です。データ本体を送る前に、送信したい STA が AP へ短い制御フレーム
**RTS**（送信要求）を送り、AP がこれに **CTS**（送信可）で応答します。CTS は
その AP の電波が届く範囲のすべての STA が受信できるため、隠れ端末の位置に
いる STA も「今からしばらくチャネルが使われる」ことを知り、その間は送信を
待ちます。こうして送信前にチャネルを予約することで、隠れ端末による衝突を
減らせます。RTS/CTS は CSMA/CA の**任意（オプション）機能**であり、常に使う
わけではなく、隠れ端末が問題になる環境などで有効化して用います。

> **試験のポイント**: 隠れ端末問題（互いの電波を検知できない STA が同時送信し、
> AP 側で衝突する問題）を CSMA/CA で緩和する仕組みは **RTS/CTS によるチャネル
> 予約**です。RTS/CTS が任意機能である点もあわせて押さえてください。
...
| **BSSID** | その AP の無線インターフェースが持つ **MAC アドレス**。BSS を一意に識別する |
...
![2.4GHz帯のチャネル重なりと1・6・11の非重複チャネル](../images/exercise10-channel-overlap.png)
...
Lightweight AP アーキテクチャでは、無線 LAN の処理機能を **AP 側と WLC 側に
...
無線特有の時間制約が厳しい処理は、AP がその場で即座にこなします。一方、
ネットワーク全体にまたがる管理的な判断は WLC に集約します。この役割分担に
よって、集中管理のメリットと無線の応答性の両立を図っています。
...
この時点では、LAP はまだ WLC の IP アドレスを知りません。そこで
**DHCP のオプション 43**（ベンダー固有情報オプション）などの仕組みを使って、
WLC の IP アドレスを学習します。アドレスが分かったら、CAPWAP トンネルを
確立して WLC に **join（参加）** します。オプション 43 以外にも、
...
> 💼 **実務では**: 「AP が WLC で "Registered / Joined" にならない」は、保守現場
> では監視アラートやユーザからの「Wi-Fi が繋がらない」報告として飛んできます。
> 一次対応の範囲は、手順書に沿って `show power inline` や `show cdp neighbors`
> などの show コマンドで次の 3 点まで切り分けることです。①PoE 電力バジェット
> 不足、②アクセス VLAN と WLC 管理 VLAN 間の L3 到達性、③DHCP オプション 43
> （または DNS）で通知した WLC の IP 誤り、の 3 点です。原因が特定できても、
> VLAN や DHCP の設定を書き換えるのは客先の許可を得てからで、切り分け結果を添えて先輩やお客様担当にエスカレーション
> するのが基本の流れになります。
...
- **コンソール / CLI**: 初回のセットアップウィザード（管理用インターフェースの
  IP アドレス設定など）で使用する
...
| **Enterprise** | **802.1X/EAP** による個別認証を行い、バックエンドの **RADIUS** サーバでユーザごとに認証する | 企業・大規模組織 |

Personal は、パスフレーズさえ知っていれば誰でも接続できます。これに対して
Enterprise は、ユーザごとに個別の資格情報（ユーザ名・証明書など）で認証します。
そのため、利用者ごとのアクセス制御や、退職者のアカウント無効化といった運用が
しやすくなります。

ここで登場する 3 つの用語は、それぞれ次のものを指します。

- **802.1X**: ポート単位でクライアントを認証する仕組みの規格
- **EAP**（Extensible Authentication Protocol）: その認証情報をやり取りする
  ためのプロトコルの総称
- **RADIUS**: ユーザ名やパスワードなどの認証情報を一元管理し、接続の可否を
  判定するサーバ
...
**4 ウェイハンドシェイク**と呼ばれる 4 回のメッセージ交換を行い、PSK から
実際の暗号鍵を安全に導出します。
...
- **SAE**（Simultaneous Authentication of Equals）: WPA3-Personal では、PSK の
  鍵交換方式が **SAE** に置き換えられます。SAE は WPA2 の 4 ウェイハンドシェイクと
...
| Personal の鍵交換 | PSK ＋ 4 ウェイハンドシェイク | **SAE**（辞書攻撃に強い・前方秘匿性あり） |
...
WLC には役割の異なる複数の論理インターフェースがあります。

| インターフェース種別 | 役割 |
|---|---|
| **management**（管理インターフェース） | WLC 自体の管理アクセスや、AP との CAPWAP トンネルを終端するインターフェース |
| **virtual**（仮想インターフェース） | Web 認証のリダイレクトなど、特殊な用途で使われる仮想的な IP アドレス |
| **dynamic**（ダイナミックインターフェース） | 無線クライアントを収容する VLAN に対応するインターフェース。WLAN ごとに紐付ける |

> **試験のポイント**: WLC のインターフェース種別（**management・virtual・
...
1. **Controller > Interfaces** で、クライアントを収容したい VLAN に対応する
   **dynamic インターフェース**をあらかじめ作成しておく
2. **WLANs > Create New** から新規 WLAN を作成する
3. **General タブ**: SSID の名前を入力し、Status を**有効（Enabled）**にする。
   さらに、先ほど作成した dynamic インターフェース（＝収容する VLAN）を対応付ける
4. **Security タブ（Layer 2）**: セキュリティ方式（WPA2 / WPA3）、認証方式
   （PSK か 802.1X）、暗号方式（AES）を選択し、PSK の場合はパスフレーズを設定する
5. **QoS / Advanced タブ**: 帯域制御や FlexConnect の設定、セッション
   タイムアウトなどの追加オプションを、必要に応じて設定する。ここでいう QoS
   （Quality of Service）とは、音声・映像など遅延に弱い通信を優先して扱う
   仕組みのことです

### スイッチ側の設定

AP や WLC を収容するスイッチのポートは、通信内容に応じて適切な VLAN で
トランクまたはアクセス設定にしておく必要があります。たとえば、AP や WLC の
管理用の VLAN（管理 VLAN）と、無線クライアント用の VLAN（複数ある場合）を、
同じアップリンク上でやり取りするとします。この場合は、そのポートを
**トランクポート**にして両方の VLAN を許可する必要があります。GUI 側で
いくら正しく WLAN やインターフェースを設定しても、スイッチ側の VLAN 設定が
誤っていれば通信は成立しません。
...
これはスイッチ全体で CDP を無効にするコマンドです。特定のインターフェースだけ
無効にしたい場合は、インターフェースコンフィグレーションモードで次を使います。
...
送信・受信の可否はインターフェースごとに個別に制御することもできます。
...
情報を与えてしまう**というリスクもあります。信頼できない相手と接続する
インターフェースでは、CDP・LLDP の無効化を検討するのが望ましい運用です。
具体的には、インターネットに面したポートや、社外の機器と接続するポートが
これにあたります。

> 💼 **実務では**: 保守現場では配線図が古い・存在しないことも珍しくなく、
> `show cdp neighbors detail` の IP・プラットフォーム・接続ポートは、目の前の
> 障害機器がどのスイッチのどのポートに繋がっているかを確認する定番の監視・
> 切り分け道具です。一方 CDP/LLDP の無効化は手順書でセキュリティ対策として
> 指示されることがありますが、Cisco の IP 電話は CDP（または LLDP-MED）で
> 音声 VLAN と PoE 給電クラスをネゴシエートしているため、確認せずに
> `no cdp enable` を投入すると通話断や給電低下という新たな障害を作り込みます。
> 設定変更を伴う判断は必ず手順書と先輩の確認を経てから行い、確認だけなら
> show コマンドの範囲で完結させるのが定石です。
...
- WPA3 は WPA2 に対し、**SAE** による PSK 方式の強化・**PMF 必須**・前方秘匿性
  という改善点を持つ
- WLC のインターフェースは management・virtual・dynamic の 3 種類があり、
  WLAN は dynamic インターフェース（＝クライアント VLAN）に対応付けて作成する
...
6. 互いの電波を検知できない位置にある STA が同時送信して AP 側で衝突する「隠れ端末問題」を、CSMA/CA で緩和する仕組みは何か。
...
5. `lldp run`
6. RTS/CTS（Request to Send／Clear to Send）によるチャネル予約

</details>

## 次のステップ

この Exercise のラボ課題「[Exercise10] ラボ: WLC と Lightweight AP による
WLAN 構築、CDP/LLDP による隣接機器確認」に進んでください。ラボでは、WLC と
LAP を CAPWAP で接続して WPA2-PSK の WLAN を構築します。そのうえで、無線
クライアントの接続確認と、CDP/LLDP による隣接機器情報の確認を実際に行います。

...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 3「IPv4 アドレッシングと
> サブネット化」 / Exercise 7「トランクと VLAN 設計」 / Exercise 9「STP と EtherChannel」 の
> 範囲から出題）。

**W1.** （Exercise 3）`/29` のサブネットマスクを 10 進表記で答えよ。また、そのサブネット
で使用可能なホストアドレス数はいくつか。

**W2.** （Exercise 7）トランクポートの既定のネイティブ VLAN 番号はいくつか。また、
トランクリンクの両端でネイティブ VLAN が一致していないと何が起きるか、一言で
答えよ。

**W3.** （Exercise 9）802.1D の STP における Hello Time・Forward Delay・Max Age の
既定値（秒）をそれぞれ答えよ。

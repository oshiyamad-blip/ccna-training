...
ことを前提にしています。しかしこの前提が崩れる状況があります。2 台の無線
クライアント（STA）が、いずれも同じ AP の電波は届く一方で、**互いの電波は
届かない位置関係**（たとえば AP を挟んで反対側にいて、間に壁がある場合）に
あるとします。このとき各 STA は相手の送信を電波として検知できないため、
...
> では監視アラートやユーザーからの「Wi-Fi が繋がらない」報告として飛んできます。
> 手順書に沿って `show power inline` や `show cdp neighbors` などの show コマンドで
> ①PoE 電力バジェット不足、②アクセス VLAN と WLC 管理 VLAN 間の L3 到達性、
> ③DHCP オプション 43（または DNS）で通知した WLC の IP 誤り、の 3 点まで切り分ける
> のが一次対応の範囲です。原因が特定できても VLAN や DHCP の設定を書き換えるのは
> 客先の許可を得てからで、切り分け結果を添えて先輩やお客様担当にエスカレーション
> するのが基本の流れになります。
...
本日のラボ課題「[Exercise10] ラボ: WLC と Lightweight AP による WLAN 構築、

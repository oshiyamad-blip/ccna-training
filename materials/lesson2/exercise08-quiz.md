...
`switchport mode trunk` でトランクに固定されており、許可 VLAN リストには
VLAN10 が含まれている。VLAN10 の PC からデフォルトゲートウェイ（R1）への
ping が失敗する。最も可能性の高い原因はどれか。

- A. `ip routing` が無効になっている
- B. PC のデフォルトゲートウェイが未設定である
- C. サブインターフェースの `encapsulation dot1q` の VLAN-ID（100）が、
     スイッチが送出する VLAN タグ（10）と一致していない
...
| Q4 | C | サブインターフェースの `encapsulation dot1q` は、スイッチ側から送出される VLAN タグと一致している必要がある。ここではスイッチが VLAN10 のタグを付けて送るのに対し、R1 側は VLAN100 のタグを期待しているため、タグが一致せずゲートウェイ宛の通信ができない。A の `ip routing` は L3 スイッチ側の設定であり Router-on-a-Stick には無関係。B は、同一サブネット内であるデフォルトゲートウェイ宛の ping にゲートウェイ設定は不要なので原因にならない。D は「トランクに固定されている」という設問の前提と矛盾する |

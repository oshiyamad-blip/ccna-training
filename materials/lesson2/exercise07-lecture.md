...
> Exercise 6「VLAN の基礎」 の範囲から出題）。

**W1.** （Exercise 15）EUI-64 方式で MAC アドレスをインターフェース ID に変換する際に
行う 2 つの操作（挿入する 16 進数の値と、反転するビット）を答えよ。

**W2.** （Exercise 6）標準範囲 VLAN と拡張範囲 VLAN の境界となる番号はいくつか。また、
FDDI / トークンリング用に予約され通常は使用しない VLAN 番号の範囲はいくつか。

**W3.** （Exercise 6）1 つのアクセスポートにデータ VLAN と音声 VLAN を共存させる

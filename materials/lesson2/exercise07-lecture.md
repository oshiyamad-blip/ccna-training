# Exercise 7 講義: トランクと VLAN 設計

> 配置先: ドキュメント `01_教材 > LESSON2 > Exercise7`
...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 6「VLAN の基礎」 / Exercise 4「IPv4 アドレッシングとサブネット化」 / Exercise 2「TCP / UDP・スイッチング動作・物理層」 の範囲から出題）。

**W1.** VLAN を分けると分割されるのは、コリジョンドメインとブロードキャストドメインの
どちらか。

**W2.** `/26` のサブネットに収容できるホスト数はいくつか。計算式も示せ。

**W3.** HTTP・HTTPS・SSH の 3 つが使うポート番号をそれぞれ答えよ。

<details><summary>解答</summary>

W1. ブロードキャストドメイン（コリジョンドメインはスイッチのポート単位で既に分割済み）
W2. 2^6 − 2 = 62 台（ホスト部 6 ビット。ネットワークアドレスとブロードキャストを除く）
W3. HTTP = TCP 80／HTTPS = TCP 443／SSH = TCP 22
...
Exercise 6 で学んだアクセスポート（access port）は、1 本のリンクにつき **1 つの VLAN**
...
> **ここがこの Exercise の山場です。** ここから先に出てくるビット数の内訳は、LESSON0 P2 で
...
| **DEI/CFI** | 1 ビット | ネットワークが混雑したときに、このフレームを優先して破棄してよいかどうかを示すビット。従来は CFI（Canonical Format Indicator）と呼ばれていた |
| **VID**（VLAN ID） | 12 ビット | フレームが属する VLAN 番号 |

フレームの並び順は次のとおりです。802.1Q タグ（TPID + TCI の 4 バイト）は
送信元 MAC アドレスと EtherType の間に挿入されます。

`宛先MAC → 送信元MAC → [ TPID(0x8100) + TCI(PCP+DEI+VID) ] → EtherType → データ …`

### VLAN ID の範囲

VID は 12 ビットのフィールドなので、表現できる値は 2^12 = **4096 通り**
（0〜4095）です。ただし、VLAN 0 と VLAN 4095 は予約されており、実際に
ユーザが使用できる VLAN 番号は **1〜4094** です。このうち **1〜1005** を
**標準範囲 VLAN**（ノーマルレンジ）、**1006〜4094** を **拡張範囲 VLAN**
（エクステンデッドレンジ）と呼びます。また **1002〜1005** は FDDI / トークンリング用に
予約された既定 VLAN で、通常のイーサネット VLAN としては使いません
（Exercise 6 で扱った内容の再確認です）。
...
- **STP（Spanning Tree Protocol、ループが起きないようスイッチ間の経路を自動調整する
  仕組み。詳しくは Exercise 9 で学びます）の不整合**: BPDU（Bridge Protocol Data Unit、
...
![トランクにおけるタグ付け／ネイティブVLANの挙動](../images/exercise07-native-vlan-mismatch.png)
...
2. **ダブルタギング（double tagging）**: 攻撃者が、外側にネイティブ VLAN の
   ...
   のみ**で成立します。対策は、ネイティブ VLAN を**ユーザが使わない
...
> 💼 **実務では**: 保守運用の現場でトランクの許可 VLAN を変更する場面は、
> たいてい手順書に沿った定型作業として先輩や上位者の指示のもとで進みます。
> 新人の最初の役割は、`show interfaces trunk` の Allowed VLANs を作業前後で
> 記録し、想定どおりの差分になっているかを確認する監視・切り分け役です。
> 手順書に明記されていない VLAN を含む変更や、管理 VLAN が乗っている
> トランクへの変更は、たとえ `add` の書き方が分かっていても自己判断で
> 実施せず、気づいた時点で先輩・上位者にエスカレーションしてください。
> 「客先で勝手に設定変更しない」は保守運用の基本ルールであり、ここで確実な
> 確認と報告を積むことが、将来構築案件へステップアップする評価にもつながります。
...
Exercise 6 では、意図しないトランク化を防ぐという文脈で DTP の名前だけに軽く触れました。
...
未使用のポートや、エンドユーザ端末を接続するアクセスポートは、次のように
...
> 💼 **実務では**: 保守運用の現場でこの設定を最初から任されることは少なく、
> 既存ポートが `access` か `trunk` か、`dynamic auto` のまま放置されて
> いないかを `show interfaces <if> switchport` の Administrative Mode /
> Operational Mode で確認するのが、まず身につけるべき仕事です。定期監視や
> 障害切り分けの中で「意図しないトランクが立っている」といった異常に
> 気づいたら、自分でモードを変更せず、手順書と先輩の判断を仰いでから
> エスカレーションします。`switchport nonegotiate` を含む恒久的なポート
> 設計の変更は、保守で確認・報告の実績を積み、構築案件を任されるように
> なってから携わる領域と考えてください。
...
ここに重大なリスクがあります。たとえば、以前のテストで VLAN を何度も作成・削除した
スイッチは、その回数だけリビジョン番号が上がっています。このスイッチを、
稼働中のネットワークへうっかり接続したとします。すると、リビジョン番号が高い
そのスイッチの VLAN データベースが正しいものとしてネットワーク全体へ広告されます。
その結果、**稼働中の VLAN データベースが古い内容で上書き・消去されてしまう**
ことがあります。
...
この Exercise のラボ課題「[Exercise07] ラボ: トランクの構築とネイティブ VLAN 不一致の

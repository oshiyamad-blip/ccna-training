...
Cisco の PVST+（VLAN ごとに STP インスタンスを持つ方式）/ RPVST+（PVST+ を
後述の RSTP で動かす方式。4 節で扱います）では、ブリッジプライオリティの
フィールドの一部を VLAN 番号の格納に使う**拡張システム ID**方式が使われます。
...
と呼び、既定では **Hello タイマ（2 秒）ごと**にルートブリッジから送出され続けます。
...
> **ここが今日の山場です。** ここから先は、スイッチごとにルートブリッジまでの
> 経路を複数比較し、コストの合計が小さい方を選ぶという計算を、1 台ずつ順番に
> 行っていきます。一度に全体を理解しようとせず、まず SW2 だけ・次に SW3 だけ、
> という具合に 1 台ずつ図と見比べながら進めれば十分です。時間をかけて構いません。
...
というハンドシェイク（合意手続き）を行うことで、Forward Delay タイマを待たずに
...
> 通信できない」という監視アラートやユーザーからの申告で、`show interfaces
...
物理インタフェースを `no switchport` で（Exercise 8 で学んだ）ルーテッドポート化して
から `channel-group` で束ね、論理インタフェース側（`interface port-channel
...
本日のラボ課題「[Exercise09] ラボ: STP の動作観察とルートブリッジの変更、EtherChannel

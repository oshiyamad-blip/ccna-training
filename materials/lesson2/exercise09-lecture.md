...
3. ポートロール（ルートポート・指定ポート・非指定ポート）とポートステートの遷移、STP の各種タイマーを説明できる
...
- **MAC アドレス**: そのスイッチ本体に固有の MAC アドレス（個々のポートに
  割り当てられたものではなく、機器 1 台を代表する 1 つのアドレスです）

Cisco の PVST+（VLAN ごとに STP を独立して動かし、VLAN ごとに別々のツリーを
作る方式）/ RPVST+（PVST+ を後述の RSTP で動かす方式。4 節で扱います）では、
ブリッジプライオリティのフィールドの一部を VLAN 番号の格納に使う
**拡張システム ID**方式が使われます。
そのため、実質的なプライオリティは次の式で表されます。
...
互いに交換し、この情報をもとにルートブリッジを決定します。BPDU にはいくつか
種類がありますが、通常の動作中に定期送出されるものを **Configuration BPDU**
と呼び、既定では **Hello タイマー（2 秒）ごと**にルートブリッジから送出され続けます。

選出のルールは、**BID が最小のスイッチ**がルートブリッジになる、という一言に
尽きます。BID はプライオリティ（上位バイト）→ MAC アドレス（下位バイト）の順で
大小を比較するため、実際の判定は次の 2 段階になります。

1. まず**プライオリティが最も小さい**スイッチを選ぶ
2. プライオリティが同値の場合は、**MAC アドレスが最も小さい**スイッチを選ぶ
...
> 💼 **実務では**: 保守運用の現場でルートブリッジの位置を自分の判断で変えることは
> ありません。`root primary` / `root secondary` の設計は構築フェーズで決まっている
> 前提のもので、保守担当が客先で勝手に触ってよい設定ではないからです。日々の仕事は
> `show spanning-tree vlan <VLAN>` で Root ID と Bridge ID を突き合わせ、設計書どおりの
> 機器がルートになっているかを定期確認すること。「特定 VLAN だけ遅い」という問い合わせ
> が来たときも、まず show コマンドで切り分けを行い、ルート位置のズレなど設計起因の
> 疑いが出た時点で自分で直そうとせず、手順書に記録した上で先輩や設計担当へ
> エスカレーションするのが基本の動きです。

## 3. ポートロールとポートステート・STP タイマー
...
| ロール | 説明 |
|---|---|
| **ルートポート（RP）** | 各非ルートスイッチが、ルートブリッジへ到達するための**最小ルートパスコスト**を持つポート。**各非ルートスイッチにつき必ず 1 つ**だけ選ばれる（ルートブリッジ自身にはルートポートが存在しない） |
...
です。コストはリンクの帯域幅に応じて、次のような値（IEEE が従来から定めている
短縮コスト値）が既定で使われます。
...
> **ここがこの Exercise の山場です。** ここから先は、スイッチごとに
> ルートブリッジまでの経路を複数比較し、コストの合計が小さい方を選ぶという
> 計算を、1 台ずつ順番に行っていきます。一度に全体を理解しようとせず、
> まず SW2 だけ・次に SW3 だけ、という具合に 1 台ずつ図と見比べながら
> 進めれば十分です。時間をかけて構いません。
...
![ポートロール選出の数値例](../images/exercise09-port-role-selection.png)

- **SW2**: SWroot への直接リンク（コスト 4）と、SW3 経由（SW3 までの 4 と
  SW3 から SWroot までの 19 を足して 4 + 19 = 23）を比較する
  → **直接リンク（コスト 4）が最小** → SWroot 側のポートがルートポートになる
- **SW3**: SWroot への直接リンク（コスト 19）と、SW2 経由（SW2 までの 4 と
  SW2 から SWroot までの 4 を足して 4 + 4 = 8）を比較する
...
STP（802.1D）のポートには次の 5 つの状態があります。このうち Disabled は、
管理者が `shutdown` した、あるいは障害でポートが無効になっている状態を指し、
**遷移の順序には含まれません**。実際に順番に遷移するのは、Disabled を除く
次の 4 状態です。
...
| タイマー | 既定値 | 内容 |
|---|---|---|
| Hello タイマー | 2 秒 | ルートブリッジが Configuration BPDU を送出する間隔 |
...
> をもとにしたポート選出の判定、802.1D のステート遷移順序、タイマー値
...
ルートポートと指定ポートは 802.1D と同じ役割のまま引き継がれます。そのうえで、
RSTP では次の 2 つのロールが追加されています。
...
リンクでは、隣接スイッチ同士が **プロポーザル（提案）/ アグリーメント（合意）**
というハンドシェイク（合意手続き）を行うことで、Forward Delay タイマーを待たずに
即座に Forwarding ステートへ移行できます。これが RSTP の高速収束の中核となる
仕組みです。
...
そのポートを強制的に **err-disabled**（エラーによる無効化）状態にする機能です。
...
> 💼 **実務では**: BPDU Guard は構築フェーズで `bpduguard default` として
> あらかじめ一括設定されているのが普通で、保守担当が新規に有効化する場面は
> ほとんどありません。実際に遭遇するのは「特定ポートがリンクダウンして
> 通信できない」という監視アラートやユーザからの申告で、`show interfaces
> status err-disabled` で該当ポートと原因を確認し、BPDU Guard によるものかを
> 切り分けるところから始まります。復旧の `shutdown` / `no shutdown` は結果を
> 左右する作業なので、手順書に定められた確認事項（利用者の持ち込み機器が
> 撤去されているか等）を満たしてから実施し、原因の特定に自信が持てない場合や
> 自己判断での復旧に不安が残る場合はそのまま先輩にエスカレーションします。
...
| プロトコル | 標準 | 概要 |
|---|---|---|
| **LACP** | IEEE 802.3ad（業界標準。現在は IEEE 802.1AX） | 他ベンダーとの相互接続も可能。CCNA では推奨されるプロトコル |
...
物理インターフェースに `channel-group` を設定すると、対応する論理インターフェース
`port-channel <番号>` が自動的に作成されます。トランクや VLAN の設定は、
基本的にこの論理インターフェース側に対して行います。
...
代表的なフラグ（ポートの状態を 1 文字で示す記号）の意味は次のとおりです。
...
物理インターフェースを `no switchport` で（Exercise 8 で学んだ）ルーテッドポート化して
から `channel-group` で束ね、論理インターフェース側（`interface port-channel
<番号>`）に `ip address` を設定すると L3 EtherChannel になります。
...
でルーテッドポート化し、論理インターフェースに直接 IP アドレスを付与します。
...
- ポートロールはルートポート（各非ルートスイッチに 1 つ。ルートブリッジには
  存在しない）・指定ポート（各セグメント 1 つ）・非指定ポートに分かれ、
  ルートパスコストの合計で決まる
...
4. ポートが err-disabled 状態になる（強制的に無効化される）

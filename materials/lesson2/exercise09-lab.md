...
![Exercise 9 トポロジ図](../images/exercise09-topology.png)
...
2. 各スイッチをクリックして [CLI] タブを開き、`enable` → `configure terminal`
   の順で特権 EXEC モード・グローバルコンフィグモードに入ってから、hostname を
   設定する
...
5. PC1・PC2 の [Desktop] → **IP Configuration** で、IP アドレス表のとおりに
   IP アドレスとサブネットマスクを設定する
6. `File > Save As` でファイルを保存する: `exercise09_氏名.pkt`（以降は、区切りの
   良いところでこまめに上書き保存しておくとよい）
...
2. PC1 から PC2 へ ping を実行し、通信が維持されていることを確認する（このトポロジ
   では PC1-PC2 間に SW1-SW3 の直結トランクなど Po1 を経由しない経路も存在するため、
   この ping 疎通だけでは EtherChannel の冗長性を証明したことにはならない点に注意する）
3. **ここが本手順の本題**: `show etherchannel summary` を再実行し、Po1 のメンバーが
   1 本に縮退しても、バンドル自体は `(SU)` のまま維持されていること（＝
   EtherChannel 内のリンク冗長性）を確認する
...
3. 手順 7〜9 で、`show etherchannel summary` のフラグ（P / SU）は何を示していましたか。
   また、Fa0/1 を shutdown してもバンドル（Po1）が `(SU)` のまま維持できた理由を、
   EtherChannel のリンク冗長性の観点から説明してください（このトポロジでは
   PC1-PC2 間の ping には Po1 以外の経路も使われ得るため、ping が維持できたこと自体を
   EtherChannel の効果として説明しないよう注意してください）。

## 提出方法

1. ファイルを `exercise09_氏名.pkt` の名前で保存する（例: `exercise09_山田太郎.pkt`）

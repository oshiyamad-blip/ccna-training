...
![Day 10 トポロジ図](../images/day10-topology.png)
...
ここが今日の山場です。ネイティブ VLAN の設定と、WLC 側の VLAN 識別子（後述の
...
1. WLC-3504 をコンソール接続する。ここまでのスイッチやルータでは自分で
   コンフィグレーションモードに入って 1 行ずつコマンドを打ってきましたが、
   WLC は初回起動時、画面に表示される質問に順番に答えていく
   **セットアップウィザード**（対話形式の設定画面）が自動的に始まります。
   主な入力項目は次のとおり

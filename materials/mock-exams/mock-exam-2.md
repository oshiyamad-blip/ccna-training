...
**Q98.** 管理者がCatalyst Centerのダッシュボードを開いたところ、あるアクセススイッチでクライアント接続数が急増し、ヘルススコアが「低下」と表示され、根本原因の候補として該当ポートのエラーカウンタ増加が提示された。この機能はCatalyst Centerの4つのワークフローのうちどれに該当するか。
...
| Q98 | D | 機器・クライアントの健全性をダッシュボードで可視化し、根本原因の候補を示すのはAssurance機能。Design/Policy/Provisionはそれぞれ設計・ポリシー定義・設定展開を担う。（→ Day19） |

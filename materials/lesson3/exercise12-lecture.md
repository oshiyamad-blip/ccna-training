# Exercise 12 講義: OSPFv2（シングルエリア）

> 配置先: ドキュメント `01_教材 > LESSON3 > Exercise12`
...
> **この Exercise の範囲**: Exercise12 はシングルエリア OSPF の**基礎**
...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 5「TCP / UDP・
> スイッチング動作・物理層」 / Exercise 9「STP と EtherChannel」 / Exercise 11「ルーティングの
> 基礎と静的ルート」 の範囲から出題）。

**W1.** （Exercise 5）TCP のスリーウェイハンドシェイクの 3 つのパケットを順に答えよ。
また、HTTPS と SSH のウェルノウンポート番号をそれぞれ答えよ。

**W2.** （Exercise 9）STP のブリッジプライオリティの既定値はいくつか。また、LACP の
`active` / `passive` の組み合わせのうち、EtherChannel が**成立しない**組み合わせは
どれか。

**W3.** （Exercise 11）ルータが経路を選ぶ際の 3 段階ロジックを順に答えよ。また、
...
Exercise11 では、管理者が 1 本ずつ手で書く静的ルートを扱いました。この章では、
...
Exercise11 で学んだ**AD（Administrative Distance）**は、同じ宛先ネットワークに対して
...
![DR/BDRによるLSAフラッディングの削減](../images/exercise12-dr-bdr-flooding.png)
...
バランシング**（Exercise11 で学んだ ECMP と同じ考え方です）を行います（既定で最大
**4 経路**まで）。
...
この Exercise のラボ課題「[Exercise12] ラボ: OSPFv2 シングルエリアの構成と経路制御」に進み、

...
> **この Exercise の範囲**: Exercise11 のシングルエリア基礎を前提に、ここでは
...
Exercise11 で学んだ OSPF のネイバー確立条件と状態遷移を、トラブルシューティングの
土台として整理し直します。
...
これは Exercise11 で学んだ一致必須項目のうち、**MTU を除いた 5 項目**です。
...
DR/BDR を選出するかどうかなど、OSPF の動作モードを決める設定です。Exercise11 で
...
![OSPF ネイバー状態遷移とトラブルシューティング](../images/exercise12-ospf-neighbor-states.png)
...
LESSON0 P5 で「町の外へ出るときに必ず通る町の出口」にたとえた、あの設定項目です。
...
VRRP は HSRP と発想がほぼ同じ（1 台が転送し、残りは待機）ですが、細部の仕様が
異なります。HSRP との比較の観点で押さえておきましょう。

- **役割の呼び方**: 転送を担当する 1 台を **Master（マスタ）**、待機するルータを
  **Backup（バックアップ）** と呼びます（HSRP の Active / Standby に対応）
- **プリエンプトの既定値**: HSRP と逆で、VRRP は**既定で有効**です。より高い
  プライオリティを持つルータが復旧すると、追加設定なしで Master の座を
  自動的に奪い返します
- **マルチキャストアドレス**: アドバタイズメント（HSRP の Hello に相当）は
  **`224.0.0.18`** 宛に送られます（HSRP v1 の `224.0.0.2`、v2 の `224.0.0.102`
  と混同しないよう区別して覚えましょう）
- **仮想 MAC アドレス**: **`0000.5e00.01XX`**（`XX` = グループ番号〔VRID〕の
  16 進表記）です。HSRP v1 の `0000.0c07.acXX` と同様、末尾にグループ番号が
  入る構造です
...
- **AVG（Active Virtual Gateway）**: グループ内で 1 台だけ選出される「司令塔」。
  端末からの ARP 要求への応答を担当し、**仮想 MAC アドレスを最大 4 台の AVF に
  割り当て**ます
- **AVF（Active Virtual Forwarder）**: AVG から割り当てられた仮想 MAC 宛の
  トラフィックを実際に転送するルータ。AVG 自身も AVF を兼ねられます
- AVG は端末からの ARP 要求に対して**端末ごとに異なる AVF の仮想 MAC を
  返す**ため、端末 A はルータ 1 を、端末 B はルータ 2 をゲートウェイとして
  使う、という形で**複数台のルータが同時にゲートウェイとして働き**ます
- 負荷分散の方式は **ラウンドロビン（既定）／重み付け（weighted）／ホスト依存
  （host-dependent、同じ端末には常に同じ AVF を返す）** の 3 つから選択できます

HSRP・VRRP では Standby／Backup のルータが平常時は待機しているだけなのに対し、
GLBP では待機役も転送に参加できるため、機器と帯域を無駄なく使えます。
...
この Exercise のラボ課題「[Exercise12] ラボ: OSPF トラブルシューティングと HSRP による

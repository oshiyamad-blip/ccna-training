...
> **本日の範囲**: Exercise12 はシングルエリア OSPF の**基礎**（隣接確立の条件・DR/BDR・
> コスト・基本設定）を扱います。MTU 不一致などのトラブル切り分けや、デフォルトルート
...
Exercise11 までで扱った静的ルートに続き、本日からは**動的ルーティングプロトコル**を扱います。
...
本日のラボ課題「[Exercise12] ラボ: OSPFv2 シングルエリアの構成と経路制御」に進み、

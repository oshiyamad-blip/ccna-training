...
7. IPv6 の静的ルートを設定し、IPv4 との違い（`ipv6 unicast-routing` の有効化・
   リンクローカルをネクストホップにする際の完全指定）を説明できる

---

## ウォームアップ（想起クイズ）

> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 14「IP サービス総合」 / Exercise 12「OSPF 応用と FHRP」 / Exercise 10「無線 LAN と検出プロトコル」 の範囲から出題）。

**W1.** DHCP の DORA の 4 つのメッセージを順に答えよ。また サーバ側・クライアント側の
UDP ポート番号も答えよ。

**W2.** HSRP で ARP 要求に応答するのはどの状態のルータか。また何のアドレスで応答するか。

**W3.** 自律型 AP と Lightweight AP ＋ WLC 構成の違いを 1 文で答えよ。

<details><summary>解答</summary>

W1. Discover → Offer → Request → Ack／サーバ = UDP 67、クライアント = UDP 68
W2. Active ルータが、仮想 MAC アドレスで応答する
W3. 自律型 AP は 1 台ごとに設定を持つが、Lightweight AP は設定・制御を WLC に集約し
AP は電波の送受信に専念する
...
IPv6 は単にアドレス数を増やしただけではありません。ヘッダを固定 40 バイトに簡素化して
ルータの処理負荷を下げ、ブロードキャストを廃止してマルチキャストに一本化するなど、
通信の効率性も見直しています。また NAT（Network Address Translation。IP アドレスを変換する
...
プレフィックス長（ネットワーク部のビット数）は、Exercise 4 で学んだ IPv4 の CIDR 表記と
同様に `/nn` で表します。IPv6 で最も一般的なサブネットサイズは **/64** で、前半 64 ビットが
ネットワーク部（プレフィックス）、後半 64 ビットが**インターフェース ID**（ホスト部）
となります。

一般的な割り当ての典型例を挙げます。ISP（インターネットサービスプロバイダ）が
...
`2001:db8::1` であれば、すでに書かれているブロックは `2001`・`db8`・`1` の 3 つです。
したがって `::` は、残りの `8 − 3 = 5` ブロック分の `0000` を表していると計算できます。
...
`fc00::/7` の範囲に属するアドレスで、実際の運用では `fd00::/8`（8 ビット目の
L ビット＝ローカル割り当てを示すビットが `1` に固定された範囲）が使われます。
IPv4 における RFC 1918 の私設アドレス（プライベートアドレス）
...
> `fe80::` 表示になっていることがあります。これを見て「グローバルアドレスが消えた」と
> 勘違いし、手が止まる新人がいますが、これは正常な状態です。まずは手順書に記載された
> 正常値と見比べて切り分け、経路自体が本当に落ちているのかどうかを判断するのが
> 監視・確認コマンドの使い方です。なお `ipv6 address fe80::1 link-local` のような
> 手動設定は、構築時の取り決めに基づくものです。保守作業の中で気づいた点があっても、
> 客先で勝手に変更してよいものではありません。変更が必要と判断した場合は独断で
> 投入せず、先輩や設計担当にエスカレーションします。
...
そのまま使って自動生成される特殊なマルチキャストアドレスです。NDP によるアドレス解決の
際に、対象ノードだけに絞り込んで問い合わせるために使われます。

### エニーキャストアドレス

エニーキャストは、**複数の機器に同じアドレスを付与**しておき、送信元から見て
経路的に最も近い 1 台にのみパケットが届く、という仕組みです。エニーキャストには
専用のアドレス範囲がなく、**ユニキャストアドレスの空間**（GUA・ULA など）から
割り当てます。そのためアドレスの書式自体は、ユニキャストアドレス（GUA・ULA・LLA の
ように、1 台だけを宛先とする通常のアドレス）と区別がつきません。同じアドレスを複数の
機器に設定し、それをエニーキャストとして扱うという運用側の取り決めによって成り立ちます。
...
> **ここがこの Exercise の山場です。** MAC アドレスからインターフェース ID を組み立てる計算は、
> 16 進数と 2 進数（LESSON0 P2 で学んだ表記です）を行き来する必要があり、最初は戸惑う
...
**手順 2**: 先頭バイトの、左（最上位ビット・MSB）から数えて **7 番目のビット**
（右から数えると 2 番目のビット）を反転させます。このビットを **U/L ビット**
（Universal/Local ビット）と呼びます。ここで行う操作は、**ビットの値を `0` → `1`
（またはその逆）に書き換えること**だけです。多くの MAC アドレスでは元の値が `0` なので、
`1` に変わります。

なぜ反転させるのかというと、**U/L ビットの値と意味の対応づけが、MAC アドレスと
インターフェース ID とで逆に決められている**からです（この方式を**修正 EUI-64**
〈modified EUI-64、RFC 4291〉と呼びます）。反転させるのはあくまで「ビットの値」で、
計算の途中で意味が変わるわけではありません。

| U/L ビットの値 | MAC アドレスでの意味 | インターフェース ID での意味 |
|---|---|---|
| `0` | ユニバーサル管理（世界で一意） | ローカルな値 |
| `1` | ローカル管理（管理者が付けた値） | グローバルに一意 |

同じ「世界で一意」という状態を、MAC アドレスでは `0`、インターフェース ID では `1` で
表します。だから変換のときに値を反転させるわけです。この意味の対応まで暗記する必要は
ありません。試験で問われるのは、次の例のとおり機械的に反転させた結果です。

### 計算例

MAC アドレス `0012.3456.789a` を例に計算してみます。

![EUI-64によるインターフェースID生成の流れ](../images/exercise15-eui64-conversion.png)
...
先頭の `00`（2 進数で `0000 0000`）の、左から 7 番目（右から 2 番目）のビットを
反転すると `0000 0010`（16 進数で `02`）になります。変化するのは右から 2 桁目の
ビットで、最後（右端・8 番目）のビットは変化しない点に注意してください。**先頭バイトが
`00` から `02` に変わる**というパターンは試験で頻出なので、必ず自分の手で一度計算
しておきましょう。
...
Cisco IOS では、インターフェースに対して次のようにコマンドを入力します。すると、
プレフィックス部分は指定した値になり、インターフェース ID 部分は EUI-64 方式で
自動生成されます。
...
リンクローカルアドレスについても同様です。明示的に設定しない限り、デフォルトでは
...
> **先に 2 つの言葉を紹介します**（詳しくは Exercise 14 で学びます。ここでは一行ずつで OK）。
>
> - **DHCP** — PC をつなぐと IP アドレスなどの設定を**サーバが自動で配ってくれる**仕組み。
>   IPv4 の世界で広く使われており、その IPv6 版が本章に出てくる **DHCPv6** です
> - **DNS サーバ** — `www.example.com` のような**名前を IP アドレスに変換してくれる**
>   案内係。どの DNS サーバを使うかも、アドレス設定と一緒にホストへ配られます
...
ホストがどの方式を使うかは、ルータが送る **RA（ルータ広告）** の中にある 2 つのフラグ
（1 ビットの旗）でホストに指示します。

| フラグ | 名称 | `1`（オン）のときホストに伝える内容 |
|---|---|---|
| **M フラグ** | Managed Address Configuration | アドレスそのものを DHCPv6 サーバから取得すること |
| **O フラグ** | Other Configuration | アドレス以外の情報（DNS サーバなど）を DHCPv6 サーバから取得すること |

両方とも `0` なら SLAAC のみ、`O` だけ `1` ならステートレス DHCPv6、`M` が `1` なら
ステートフル DHCPv6 という組み合わせになります。

> 💼 **実務では**: 「特定の Android 端末だけ IPv6 アドレスが振られない」という
> 問い合わせは、保守運用の現場で実際に上がってくる相談です。原因の多くは
> RA の M/O フラグ設計にあります（Android はステートフル DHCPv6 に非対応という
> 制約があるためです）。この場合も、保守担当がその場で設定をいじることはしません。
> 端末側の設定（自動/DHCP）を手順書どおりに確認して事象を切り分け、設計に関わる話
> として先輩やネットワーク設計チームにエスカレーションするのが基本です。こうした
> 切り分けと報告を丁寧に積み重ねた先に、RA の設計そのものを任される構築案件への
> ステップアップがあります。

### ホスト側の設定

PC 側では、IPv6 の設定方法として「自動（SLAAC）」または「DHCP」のいずれかを
選択します。ルータ側は、`ipv6 unicast-routing` を有効化したうえで、インターフェースに
IPv6 アドレスを設定します。これだけで、RA のパラメータを個別に設定しなくても
**デフォルトで RA の送出が始まります**。そのため追加の設定なしで、SLAAC による
アドレス配布が機能します。逆に `ipv6 unicast-routing` を入力し忘れていると RA が
送出されず、SLAAC は動作しません。
...
ただし、**リンクローカルアドレス宛てに ping を打つ場合**は注意が必要です。
そのアドレスがどのリンク（インターフェース）に属するかが一意に決まらないため、
**出力インターフェースを明示的に指定する**必要があります。
...
Exercise5 で学んだルーティングの考え方を、IPv6 に当てはめます。経路選択のロジック
（ロンゲストマッチ → AD → メトリック）は IPv4 とまったく同じで、変わるのは
アドレスの書き方と、いくつかの前提コマンドだけです。

### 前提設定: ipv6 unicast-routing

IPv4 のルータは、デフォルトでパケット転送が有効になっています。一方 IPv6 では、
グローバルコンフィグモードで次のコマンドを実行して**IPv6 のユニキャスト
ルーティングを明示的に有効化**しない限り、ルータは IPv6 パケットを転送しません。

```
R1(config)# ipv6 unicast-routing
```

これは IPv4 の `ip routing`（デフォルトで有効）との重要な違いです。Exercise8 で学んだ
...
この Exercise の第 3 章で学んだ**リンクローカルアドレス**（`fe80::/10`）は各リンク（インターフェース）ごとに
...
この Exercise のラボ課題「[Exercise15] ラボ: IPv6 アドレッシング — 静的設定と SLAAC の
構成」に進み、ここまでに学んだ静的アドレス設定と SLAAC の動作を、実際にルータと PC に
構成しながら確認してください。

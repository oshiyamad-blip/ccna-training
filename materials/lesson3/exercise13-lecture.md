...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 12「OSPF 応用と FHRP」 / Exercise 10「無線 LAN と検出プロトコル」 / Exercise 8「VLAN 間ルーティング」 の範囲から出題）。

**W1.** OSPF のネイバーが ExStart / Exchange で止まっているとき、
最初に疑う原因は何か。

**W2.** 隣接機器の機種名や接続ポートまで詳しく確認する CDP のコマンドは何か。

**W3.** Router on a Stick と L3 スイッチ（SVI）を比べたとき、
L3 スイッチが有利な点を 1 つ挙げよ。

<details><summary>解答</summary>

W1. インターフェースの MTU 不一致（DBD 交換が完了できない）
W2. `show cdp neighbors detail`
W3. ASIC によるハードウェア転送のため高速で、
1 本のトランクに帯域が集中するボトルネックも生じない

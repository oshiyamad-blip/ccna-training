# Exercise 13 講義: NAT（ネットワークアドレス変換）

> 配置先: ドキュメント `01_教材 > LESSON3 > Exercise13`
...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 7「トランクと VLAN 設計」 / Exercise 5「ルーティングの基礎と静的ルート」 / Exercise 12「OSPF 応用と FHRP」 の範囲から出題）。

**W1.** （Exercise 7）802.1Q トランクにおいて、ネイティブ VLAN のフレームは VLAN タグが付与されて送信されるか。また Cisco スイッチのデフォルトのネイティブ VLAN 番号はいくつか。

**W2.** （Exercise 5）スタティックルートのデフォルトの管理距離（AD）はいくつか。同じ宛先に OSPF の経路（AD 110）も存在する場合、どちらのルートがルーティングテーブルに採用されるか。

**W3.** （Exercise 12）HSRP のデフォルトの Hello タイマーと Hold タイマーはそれぞれ何秒か。また Active ルータ障害時、Standby ルータが Active に昇格するまでの目安時間は？
...
![NAT の 4 用語（inside/outside × local/global）](../images/exercise13-nat-terminology.png)
...
この Exercise のラボ課題「[Exercise13] ラボ: 静的 NAT・動的 NAT・PAT の構成と変換テーブルの観察」に

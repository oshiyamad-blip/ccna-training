...
Exercise5 では、管理者が 1 本ずつ手で書く静的ルートを扱いました。この章では、
ルータ同士が経路情報を交換して自動的にテーブルを作る動的ルーティングに視点を移し、
その中で OSPF がどの位置にいるプロトコルなのかを整理します。

### ディスタンスベクタとリンクステートの違い

**動的ルーティングプロトコル**は、大きく 2 つの方式に分類されます。
...
Exercise5 で学んだ**AD（Administrative Distance）**は、同じ宛先ネットワークに対して
...
この Exercise では **OSPFv2 のシングルエリア**構成を扱います。「シングルエリア」とは、
ネットワーク内の全ルータを**エリア 0（バックボーンエリア）**という 1 つのエリアだけに
所属させる、最もシンプルな OSPF の構成です。複数のエリアに分割するマルチエリア構成は
CCNA 200-301 v1.1 の出題範囲外のため、本コースでは扱いません。
...
前章で、OSPF は「地図（LSDB）」を共有するプロトコルだと分かりました。では、その地図を
交換する相手（ネイバー）はどうやって見つけ、どんな条件がそろえば情報交換を始められる
のでしょうか。この章ではその手順を順に追います。
...
- **スタブフラグ**（スタブエリアの設定が一致していること。スタブエリアは
  ネットワークを複数のエリアに分割するマルチエリア構成で使う概念で、この Exercise の
...
前章の一致条件の中で、Router ID だけは「一致」ではなく「一意」が求められる特別な値でした。
この章では、その Router ID がどのように決まるのかと、マルチアクセス網でネイバー関係の
数を抑えるために置かれる DR/BDR の選出を扱います。
...
ここで **NBMA（Non-Broadcast Multi-Access）** とは、同一区間に複数の機器が
つながっていながら、ブロードキャストが使えない接続形態のことです。その代表例が
**フレームリレー網**で、かつて広域網（WAN）で広く使われた回線サービスです。
現在の新規構築ではほとんど使われませんが、OSPF のネットワークタイプやタイマーの
既定値を説明する際に必ず出てくるため、名前と「ブロードキャストが使えない
マルチアクセス網」という性質だけ押さえておけば十分です。
...
![DR/BDRによるLSAフラッディングの削減](../images/exercise11-dr-bdr-flooding.png)
...
> **ここがこの Exercise の山場です。**「優先度が高いルータが後から参加すれば、
> そちらが DR になるはず」と考えてしまいがちですが、実際の動作はそうではありません。
> 時間をかけて読んで構いません。
...
ここまでで、ネイバーが成立して LSDB が共有されるところまでを見てきました。次は、
その地図の上で「どの経路を最短とみなすか」を決める物差し、つまりコストの話に進みます。
...
バランシング**（Exercise5 で学んだ ECMP と同じ考え方です）を行います（既定で最大
**4 経路**まで）。
...
動作原理が一通りそろったので、この章では実際にルータへ OSPF を設定するコマンドを
順に見ていきます。ラボ手順書で打つコマンドは、すべてこの章に出てきます。
...
PC やサーバを収容するだけの LAN 側インターフェースには、ネイバーが現れる必要が
ありません。そのため、**パッシブインターフェースにするのがセキュリティ・効率上の
ベストプラクティス**です。不要な Hello 送信を止められるうえ、不正なルータが
OSPF に参加してくるのも防げます。
...
設定を投入したら、意図どおりに動いているかを自分の目で確かめる必要があります。
この章では、状態を確認する show コマンドと、ネイバーができないときの切り分けの
勘所をまとめます。
...
この Exercise で扱った要点を、あとから見返せる形で整理します。

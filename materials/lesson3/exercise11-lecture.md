# Exercise 11 講義: ルーティングの基礎と静的ルート
...
状態を伝え合うための制御用プロトコルです。Exercise4 で学んだ ICMPv6 は、この ICMP の
IPv6 版にあたります。
...
動的ルーティングプロトコルの詳細は、次の Exercise 12・13 で扱います。
...
以下は、Exercise11 ラボと同じ宛先（LAN3・`192.168.3.0/24`）に対して 4 つの経路候補が
存在する場合に、実際にどの順序でどの経路まで絞り込まれるかを示した例です。

![経路選択の3段階ロジック（ロンゲストマッチ→AD→メトリック）の絞り込み例](../images/exercise11-route-selection-logic.png)
...
Exercise4 で学んだ**リンクローカルアドレス**（`fe80::/10`）は各リンク（インターフェース）ごとに
...
- この Exercise の最重要コマンドセット: `ip route` / `ipv6 route`（設定）、
...
この Exercise のラボ課題「[Exercise11] ラボ: 静的ルートとフローティングスタティックの構成」に進み、

...
- （発展）ルータを 1 台足して IPv6 の静的ルート・デフォルトルートを設定し、
  Exercise5 で学んだ IPv4 の静的ルートとの違いを確かめられる
...
必須の手順ではありません。**提出物にも採点にも含めません**。手順 7 まで終えて
時間が余った人だけ取り組んでください。ここまでの構成に **R2 を 1 台足して**、
Exercise5 で IPv4 について学んだ静的ルートを、そのまま IPv6 で組んでみます。

1. Router 2911 を 1 台追加して `R2` とし、R1 Gi0/2 ⇔ R2 Gi0/0 を接続する。
   さらに R2 Gi0/1 に PC5 を接続する（スイッチは挟まなくてよい）
2. 追加分のアドレスを設定する

   | 機器 | インターフェース | IPv6 アドレス |
   |---|---|---|
   | R1 | Gi0/2 | `2001:DB8:0:12::1/64` |
   | R2 | Gi0/0 | `2001:DB8:0:12::2/64` |
   | R2 | Gi0/1 | `2001:DB8:0:3::1/64` |
   | PC5 | NIC | `2001:DB8:0:3::10/64`（GW: `2001:DB8:0:3::1`） |

3. **R2 でも `ipv6 unicast-routing` を有効化する**（忘れると転送されない）
4. R1 に、R2 配下のセグメント宛の IPv6 静的ルートを設定する

   ```
   R1(config)# ipv6 route 2001:DB8:0:3::/64 2001:DB8:0:12::2
   ```

5. R2 に、**復路**の IPv6 静的ルートを設定する（片方向だけでは ping は返って
   こない。Exercise5 で扱った往路・復路の話と同じです）

   ```
   R2(config)# ipv6 route 2001:DB8:0:1::/64 2001:DB8:0:12::1
   R2(config)# ipv6 route 2001:DB8:0:2::/64 2001:DB8:0:12::1
   ```

6. PC1 から PC5 へ ping し、`show ipv6 route` で `S` コードの経路を確認する
7. 発展として、R2 側の 2 本の静的ルートを IPv6 デフォルトルート 1 本に
   まとめ直し、同じように疎通することを確かめる

   ```
   R2(config)# ipv6 route ::/0 2001:DB8:0:12::1
   ```

8. リンクローカルアドレスをネクストホップにする書き方も試してみる。
   `show ipv6 interface brief` で R1 Gi0/2 の `fe80::` アドレスを調べ、
   **出力インターフェースを併記した完全指定**で設定する（併記しないと
   設定が受け付けられない、または意図しないリンクを指してしまう）

   ```
   R2(config)# no ipv6 route ::/0 2001:DB8:0:12::1
   R2(config)# ipv6 route ::/0 GigabitEthernet0/0 fe80::<R1 の値>
   ```

> 発展課題を試した場合は、**別名で保存**してください（`exercise15_発展_氏名.pkt`）。
> 提出する `exercise15_氏名.pkt` は手順 7 までの状態のままにしておきます。

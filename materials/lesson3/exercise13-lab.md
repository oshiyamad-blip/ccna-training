...
2. 次の機器を配置し、置いた直後にそれぞれ名前を付ける
   ...
   - Router **2911** × 2 → `R1`（NAT 変換ルータ）／ `R2`（ISP 役）
   - Switch **2960** × 1 → `SW1`
   - PC × 3 → `PC1` / `PC2` / `PC3`
   - Server × 1 → `Srv1`（外部 Web サーバ役）

3. 接続ツール（稲妻アイコン）で、次のとおり結線する

   | 機器A・ポート | ケーブル | 機器B・ポート |
   |---|---|---|
   | PC1 Fa0 | ストレート | SW1 Fa0/1 |
   | PC2 Fa0 | ストレート | SW1 Fa0/2 |
   | PC3 Fa0 | ストレート | SW1 Fa0/3 |
   | SW1 Fa0/24 | ストレート | R1 Gi0/0 |
   | R1 Gi0/1 | ストレート | R2 Gi0/1 |
   | R2 Gi0/0 | ストレート | Srv1 Fa0 |

4. すべてのリンクが**緑**になるまで待つ
   （スイッチが絡む区間は STP の収束に 30 秒ほどかかります）

> R1 の外側（Gi0/1）から先が「インターネット」、R2 の先の Srv1 が外部サーバです。
> この内と外の区切りが、NAT の `inside` / `outside` にそのまま対応します。
...
## 手順 3: inside / outside インターフェースの割り当て（10 分）
...
4. PC1 から `ping 198.51.100.8` を実行し、続けて PC2 からも `ping 198.51.100.8` を実行する
   （**手順 5〜6 は間を空けず連続で実行してください**。変換エントリは一定時間
   無通信が続くとタイムアウトで解放されるため、間隔が空くと手順 7 の枯渇が
   再現できないことがあります）
...
## 手順 6: PAT（NAT Overload）の構成と観察（30 分）
...
1. R1 の Gi0/1（外部インターフェース）のアドレスに、内部ホストすべてを
   ...
   （ACL 1 は手順 5 で作成したものをそのまま再利用します）
...
| 動的 NAT でプールが枯渇するはずの PC3 が成功してしまう | PC1/PC2 の変換エントリがタイムアウトで解放された可能性があります。手順 5〜6 を間を空けず連続で実行し直してください |

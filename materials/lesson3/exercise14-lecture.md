# Exercise 14 講義: NAT（ネットワークアドレス変換）
...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 7「トランクと VLAN 設計」 / Exercise 11「ルーティングの基礎と静的ルート」 / Exercise 13「OSPF 応用と FHRP」 の範囲から出題）。

**W1.** （Exercise 7）802.1Q トランクにおいて、ネイティブ VLAN のフレームは VLAN タグが付与されて送信されるか。また Cisco スイッチのデフォルトのネイティブ VLAN 番号はいくつか。

**W2.** （Exercise 11）スタティックルートのデフォルトの管理距離（AD）はいくつか。同じ宛先に OSPF の経路（AD 110）も存在する場合、どちらのルートがルーティングテーブルに採用されるか。

**W3.** （Exercise 13）HSRP のデフォルトの Hello タイマーと Hold タイマーはそれぞれ何秒か。また Active ルータ障害時、Standby ルータが Active に昇格するまでの目安時間は？
...
IPv4 アドレスは 32 ビット（LESSON0 P2 で学んだ 2 進数 32 桁）で表現され、
組み合わせは約 43 億個しかありません。インターネットの急拡大により
...
| 範囲 | 由来クラス | 用途の目安 |
|---|---|---|
| `10.0.0.0/8` | クラスA（1 個） | 大規模組織全体 |
| `172.16.0.0/12` | クラスB 16 個分 | 中規模拠点 |
| `192.168.0.0/16` | クラスC 256 個分 | 小規模 LAN・家庭 |

> `/12` や `/16` はそれぞれ複数のクラスB・クラスC ネットワークをまとめた範囲であり、
> クラスの既定マスク（クラスB は `/16`、クラスC は `/24`）とは別物です。
...
### Exercise 1 の原則の例外としての NAT

Exercise 1 で「L3 ヘッダ（IP アドレス）は原則、送信元から宛先まで変わらない（NAT を除く）」
...
この表だけではややこしく感じるかもしれません。同じ 4 用語を「**誰のアドレス
か**」×「**どこで通用する表現か**」の 2 軸で読み直すと、次のように整理できます。

| | 内部ネットワークで通用する表現 | 外部ネットワークで通用する表現 |
|---|---|---|
| **内部の機器**のアドレス | 内部ローカル（inside local） | 内部グローバル（inside global） |
| **外部の機器**のアドレス | 外部ローカル（outside local） | 外部グローバル（outside global） |

覚え方は 2 つだけです。**inside / outside ＝ そのアドレスの持ち主がどちら側の
機器か**、**local / global ＝ そのアドレス表現がどちら側のネットワークで通用
するか**。たとえば「外部ローカル」は、**外部の機器が、内部ネットワークの中では
別の姿（アドレス）で見えている**状態を指す、と読み解けます。
...
通信相手の外部サーバが `198.51.100.8` だとすると、次のように対応します。

| 用語 | 値 | 象限 |
|---|---|---|
| inside local | 192.168.1.10 | 内側×ローカル |
| inside global | 203.0.113.10 | 内側×グローバル |
| outside local | 198.51.100.8 | 外側×ローカル |
| outside global | 198.51.100.8 | 外側×グローバル |
...
![NAT の 4 用語（inside/outside × local/global）](../images/exercise14-nat-terminology.png)
...
- `show ip nat translations` を実行すると、静的エントリは **Pro（プロトコル）列**
  と外部側の 2 列が `---` と表示され、通信の有無にかかわらず常時表示され続ける
  （この出力に経過時間の列はありません）
...
ACL は、送信元や宛先のアドレスを条件に通信を許可（permit）・拒否（deny）する
条件リストです。ACL 自体は Exercise17 で本格的に扱うため、ここでは動的 NAT に
必要な「対象アドレスを 1 行で指定する」最小限の書き方だけ押さえれば十分です。
...
- `access-list 1` で変換対象とする内部アドレス範囲を許可（`0.0.0.255` は
  Exercise12 で学んだワイルドカードマスク）
...
Router# show ip nat translations
Pro Inside global          Inside local         Outside local        Outside global
tcp 203.0.113.1:1025       192.168.1.10:1025    198.51.100.8:80           198.51.100.8:80
tcp 203.0.113.1:1026       192.168.1.11:1030    198.51.100.8:80           198.51.100.8:80
icmp 203.0.113.1:512       192.168.1.10:512     198.51.100.8:512          198.51.100.8:512
```
...
- PAT は外部から内部への通信開始を基本的に許さない（内部発信のセッションに
  対してのみ**ステートフル**に変換する。通信の状態を記憶し、内部から始まった
  通信の戻りだけを通す、という意味です）ため、簡易的な保護機能にもなる

> **試験のポイント**: 「1 つのグローバルアドレスで多数の内部ホストを収容する
> 方式はどれか」という分類問題、`overload` キーワードの有無・位置を問う設定問題、
> および「ICMP にはポート番号が無くクエリ識別子が使われる」という引っかけが頻出です。

> 💼 **実務では**: 保守現場でこの構成に触れるのは主に障害対応の場面です。
> 「公開サーバに外部からアクセスできない」という一次切り分けでは、まず
> `show ip nat translations` で静的 NAT のエントリが想定どおり存在するか、
> グローバル IP 宛の戻り経路が ISP 側まで通っているかを確認します。設定の
> 追加・変更が必要だと判断した場合は自己判断で手を入れず、切り分けの結果を
> 添えて先輩やリーダーへエスカレーションするのが現場のルールです。DMZ
> （公開サーバを社内 LAN から隔離して置く区画。Exercise 16 で学びます）を
> 含むこうした構成を自分で設計できるようになるのは、保守で信頼を積んだ先に
> 任される構築案件の領域です。
...
5. `show ip nat translations` の出力で、Pro（プロトコル）列が `---` と表示され、
   通信の有無にかかわらず常時残り続けるのはどの方式のエントリか。
...
5. 静的 NAT（プロトコルやポート番号を伴わない 1 対 1 の固定変換のため、Pro 列が
   `---` になる）
...
この Exercise のラボ課題「[Exercise14] ラボ: 静的 NAT・動的 NAT・PAT の構成と変換テーブルの観察」に

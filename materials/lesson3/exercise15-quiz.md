# Exercise 15 小テスト（LESSON3 まとめテスト）: IP サービス ほか LESSON3 総合
...
| Q11 | C | GLBP は AVG が複数の AVF に仮想 MAC を割り当てて負荷を分散させ、**単一のグループ内で**複数台が同時に転送できる唯一の FHRP。なお HSRP・VRRP でも、グループを複数作って VLAN ごとに Active / Master を分ければ機器単位の負荷分散は可能で、「1 グループ内で同時転送できる」点が GLBP 固有である。VRRP は業界標準（RFC 5798）、HSRP は Cisco 独自 |

...
**127.0.0.0/8** はクラスA の範囲内にありますが、実際にはホストへ割り当てず
...
グローバルアドレスに変換する必要があります。NAT/PAT の設定詳細は Exercise 14 で
...
> 繋がらない」という一次切り分け依頼が来たときです。`ipconfig` や `show ip
> interface brief` などの確認コマンドで自宅ルータ側のアドレス帯（多くが既定の
> `192.168.1.0/24`）と社内の払い出し帯を突き合わせ、重複の疑いが見えても、その場で
...
> 💼 **実務では**: 保守現場で `/30` か `/31` かを気にするのは、主に
> `show ip interface brief` や `show running-config interface` で WAN リンクの
> 状態を確認し、構成台帳や手順書に記載された値と一致しているかを照合する場面です。
> マスクが想定と違っていても、客先で勝手にその場で直すことはせず、事象を記録して
> 先輩やお客様担当にエスカレーションするのが客先常駐の基本ルールです。こうした
...
本日のラボ課題「[Exercise03] ラボ: VLSM によるアドレス設計とルーティング」に進み、

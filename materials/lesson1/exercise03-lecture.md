# Exercise 3 講義: Cisco IOS の基本操作とデバイス初期設定

> 配置先: ドキュメント `01_教材 > LESSON1 > Exercise3`
...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 2「TCP / UDP・スイッチング動作・物理層」 の範囲から出題）。

**W1.** スイッチが MAC アドレステーブルに登録するのは、受信したフレームの「送信元 MAC」と
「宛先 MAC」のどちらか。また何と対応づけて登録するか。

<details><summary>解答</summary>

W1. 送信元 MAC アドレスを、そのフレームを受信したポートと対応づけて登録する
...
まず CLI（コマンドラインインターフェース）へ接続する方法を理解する必要があります。
...
| サブ設定（インターフェース） | `Router(config-if)#` | 個別インターフェースの設定 |
...
Router(config-if)#            ← サブ設定モード（インターフェース）
...
- 特権 EXEC から `disable` を実行するとユーザ EXEC（`Router>`）に戻ります。一方、
  特権 EXEC で `exit`（または `logout`）を実行してもユーザ EXEC には戻りません。
  EXEC セッションそのものが終了します（コンソールでは再びログインプロンプトに戻り、
  SSH/Telnet では接続が切断されます）。
- 語呂で覚えるなら「`end` = 一気に `#` まで、`exit` = 一段だけ（ただし特権 EXEC
  の `exit` はセッション終了）、`disable` = `#` から `>` へ」です。

![IOS コマンドモードの状態遷移](../images/exercise03-ios-command-modes.png)
...
ここで出てくる**ハッシュ**とは、元のデータを決まった計算式で別の値に変換したものです。
変換後の値から元のデータを逆算できない（不可逆である）点が特徴で、パスワードそのもの
ではなくハッシュを保存しておけば、設定を見られても元のパスワードは分かりません。
このあと出てくる MD5・PBKDF2・scrypt という名前は、いずれもハッシュの計算方式
（アルゴリズム）の名称です。

- `enable secret`: パスワードを**ハッシュ**として running-config に保存します。
  `show running-config` を見ても元のパスワードは分かりません。CCNA では
  「MD5 相当のハッシュ（Type 5）」として覚えますが、実際にどの方式が使われるかは
  **機種と IOS のバージョンによって異なります**（現行機では後述の Type 8 / Type 9 が
  既定のこともあります）。
...
> 事故です。実際の保護は `enable secret`（現行 IOS では **Type 8 = PBKDF2**、
> **Type 9 = scrypt**）側が担っています。パスワードを含む設定を社外・社内問わず
> 共有してよいかは自己判断せず、迷ったら手順書を確認するか、マスクの要否を
> 先輩に確認してからにしましょう。
...
> **ここで一区切りです。** 第 3 章までが「機器そのものの基本設定」、第 4 章からは
> 「離れた場所から機器を管理するための設定」に進みます。ここから先は **IP アドレスの
> 知識**（次の Exercise 4 で詳しく学びます）を使うため、初見で難しく感じたら、いったん
> コマンドを写して動かすだけで OK です。**Exercise 4 を終えたあとにこの章へ戻って読み直す**と、
> 「なぜこのアドレスとサブネットマスクなのか」まで腹落ちします。
...
ポートには IP を設定しません。代わりに、**SVI（Switch Virtual Interface）**という
仮想インターフェースに管理用の IP アドレスを設定します。SVI とは、**VLAN**
（スイッチのポートを論理的にグループ分けする仕組み。詳しくは Exercise 6 で学びます）
1 つひとつに対応して作られる、スイッチ内部の仮想的なインターフェースのことです。
...
自分とは異なるサブネットにいる端末からスイッチを管理する場合、注意が必要です。
L2 スイッチはルーティング機能を持たないため、そのままでは戻りの通信をどこへ
送り出せばよいか分かりません。そこで、外へ出るときの出口として**デフォルト
ゲートウェイ**（LESSON0 P5 で学んだ、自分のネットワークの外へ出るときに通る出口です）
を明示的に設定します。
...
ルータは L3（ネットワーク層）で動作する機器なので、**物理インターフェースに直接**
IP アドレスを設定します。
...
`show ip interface brief` は、各インターフェースの IP アドレスと **Status**（物理層の
...
インターフェースには `description` コマンドで用途のメモを付けられます（例:
...
> **ここがこの Exercise の山場です。** SSH の有効化は、前提設定を正しい順番で積み重ねる
...
鍵の長さ（モジュラス）を聞かれたら、**1024 ビット以上**を指定します。SSH バージョン 2
を使うには最低 768 ビットが必要ですが、強度の面から実務では 1024 ビット以上にします。

```
How many bits in the modulus [512]: 1024
```

なお、すでに RSA 鍵が生成済みの機器で `crypto key generate rsa` を実行した場合の
挙動は環境によって異なります。「鍵がすでに存在する（置き換えるか）」という確認が
出ることもあれば、エラーで先に進めないこともあります。鍵を作り直したい場合は、
先に既存の鍵を削除してから生成し直します。削除には `crypto key zeroize rsa` を使います。
...
> 任される場合も、手順書の作業前確認・戻し手順に従います。`transport input ssh` /
> `login local` に切り替える間は、コンソールセッションを 1 本開けたままにしておきます。
> そして別端末で SSH ログインが成功するのを確認してから閉じるのが定石です。打ち間違いで
> 「唯一のリモート管理経路から自分を締め出す」事故が典型的なためです。判断に迷う
...
| コマンド | 表示内容 |
|---|---|
| `show ip interface brief` | 各インターフェースの IP アドレスと状態（Status/Protocol） |
...
| `show interfaces` | 各インターフェースの詳細な統計情報（エラー数・速度・デュプレックスなど） |
...
- スイッチの管理 IP は SVI（`interface vlan`）に設定し、ルータは物理インターフェースに
...
2. `enable secret` と `enable password` の両方が設定されている場合、どちらが
   有効になるか。
3. スイッチの管理用 IP アドレスは、どのインターフェースに設定するか。
...
3. SVI（`interface vlan 1` などの VLAN インターフェース）

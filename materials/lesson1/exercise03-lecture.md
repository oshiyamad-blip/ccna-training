# Exercise 3 講義: IPv4 アドレッシングとサブネット化

> 配置先: ドキュメント `01_教材 > LESSON1_ネットワーク基礎 > Exercise03`
...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 2「Cisco IOS の基本操作と
> デバイス初期設定」の範囲から出題）。
...
**W3.** ルータの物理インターフェースに IP アドレスを設定しても、忘れると通信できない
ままになる設定コマンドは何か。
...
W3. `no shutdown`（ルータの物理インターフェースは既定で administratively down のため）
...
共有する **PAT（Port Address Translation）** です。NAT/PAT の設定詳細は Exercise 14 で
...
![VLSM設計: 大きいブロックから連続して割り当てる](../images/exercise03-vlsm-block-allocation.png)
...
この Exercise のラボ課題「[Exercise03] ラボ: VLSM によるアドレス設計とルーティング」に

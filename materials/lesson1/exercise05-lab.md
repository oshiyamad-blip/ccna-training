# Exercise 5 ラボ手順書: 静的ルートとフローティングスタティックの構成

> 配置先: ドキュメント `02_ラボ手順書 > LESSON1 > Exercise5`
> 所要時間の目安: 2.5 時間 ／ 使用ツール: Cisco Packet Tracer 9.x

## ゴール

- ルータ 3 台を直列（R1-R2-R3）に接続した環境で、IPv4 の静的ルートと
  デフォルトルートを設定し、端末間（PC1-PC3）の全区間疎通を実現できる
- R1-R3 間に冗長リンクを追加し、フローティングスタティックによる
  バックアップ経路への自動切り替えを検証できる
- `show ip route` の出力から、実際に使われている経路とその AD・
  ネクストホップを読み取れる
...
> **注記**: 障害シミュレーション（手順 10）との整合のため、本ラボでは R1-R3 の
...
| 機器 | インターフェース | IPv4 アドレス | 備考 |
|---|---|---|---|
| PC1 | NIC | 192.168.1.10/24（GW: .1） | LAN1 |
| R1 | Gi0/0 | 192.168.1.1/24 | LAN1 側 |
| R1 | Gi0/1 | 10.0.12.1/30 | バックアップ経路（R2 方向） |
| R1 | Gi0/2 | 10.0.13.1/30 | 主経路（R3 方向） |
| R2 | Gi0/0 | 10.0.12.2/30 | バックアップ経路（R1 方向） |
| R2 | Gi0/1 | 10.0.23.1/30 | バックアップ経路（R3 方向） |
| R3 | Gi0/0 | 10.0.23.2/30 | バックアップ経路（R2 方向） |
| R3 | Gi0/1 | 192.168.3.1/24 | LAN3 側 |
| R3 | Gi0/2 | 10.0.13.2/30 | 主経路（R1 方向） |
| PC3 | NIC | 192.168.3.10/24（GW: .1） | LAN3 |
...
4. ストレートケーブルで次のように接続する
   ...
   - R1 Gi0/2 ⇔ R3 Gi0/2（主経路として使用する直結リンク。次の項目 5 で
     アドレスを設定する）
5. 各ルータで CLI を開き、IP アドレス表に従ってインターフェースへ IPv4
...
2. PC3 も同様に、`192.168.3.10` / `255.255.255.0` / GW `192.168.3.1` を設定する
3. ファイルを保存する: `File > Save As` → `exercise05_氏名.pkt`
...
3. R2 に、両端の LAN 向けの静的ルートを設定する（この経路は手順 8 で
...
2. PC1 の Command Prompt から、PC3 への疎通を確認する
...
## 手順 6: デフォルトルートの演習（15 分）

1. R3 に、LAN3（`192.168.3.0/24`）以外の宛先を R2 へ送るデフォルトルートを
   **追加**し、ロンゲストマッチの効き方を確認する。手順 4 で設定した
   `192.168.1.0/24` 向けの静的ルートは削除しません

   ```
   R3(config)# ip route 0.0.0.0 0.0.0.0 10.0.23.1
   ```

   デフォルトルート（`0.0.0.0/0`）はプレフィックス長が 0 で最も緩い一致のため、
   より具体的な `192.168.1.0/24` の静的ルートが残っている限り、LAN1 宛の
   パケットは引き続き主経路（`10.0.13.1`）で転送されます。
...
## 手順 10: 障害シミュレーションと自動切り替えの確認（15 分）
...
3. PC1 の Command Prompt から `ping -t 192.168.3.10` を実行して継続 ping を
   開始する（Packet Tracer の素の `ping` は既定で 4 発しか送信されないため、
   `-t` オプションで停止するまで送信し続けるようにする）。その状態のまま
   手順 10-1 の `shutdown` を別ウィンドウで実行し、Reply が途切れてから
   再開するまでを観察する。再開を確認したら `Ctrl+C` で停止し、表示される
   `Lost = N` から失われたパケット数を記録する
...
2. `show running-config | include route` で、静的ルート・デフォルトルート・
   ...
   （`include route` と書くと、静的ルート・デフォルトルート・フローティング
   スタティックの行がまとめて表示される）
...
2. R1 の Gi0/2 を `shutdown` した前後で `show ip route` の該当行がどう変化したか
   （ネクストホップ・AD）を記録し、手順 10-3 の `ping -t` を `Ctrl+C` で停止した
   際に表示される `Lost = N` から、PC1→PC3 の ping が復旧するまでに失われた
   パケット数を報告せよ。
...
2. 手順 5・10 の疎通確認結果（スクリーンショット可）と観察レポートを
   課題の**コメント**に貼る
3. 課題の状態を「処理済み」に変更する

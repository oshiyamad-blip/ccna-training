# Exercise 5 講義: TCP / UDP・スイッチング動作・物理層

> 配置先: ドキュメント `01_教材 > LESSON1_ネットワーク基礎 > Exercise05`
...
> デバイス初期設定」 / Exercise 4「IPv6 アドレッシング」 の範囲から出題）。
...
![スイッチのフレーム転送判断ロジック](../images/exercise05-switch-forwarding-decision.png)
...
クロスケーブルになる点が試験の引っかけになりやすいので注意しましょう。実際の
判断基準はポートの種類（MDI か MDI-X か）で、PC とルータのポートはどちらも
MDI のため、同じ MDI 同士を直結する PC — ルータもクロスケーブルが必要になります。
...
アプリケーション」の積み重ねでできています。**この積み重ねのどこまでを事業者が
提供してくれるか**で、クラウドサービスは 3 種類に分かれます。
...
本日のラボ課題「[Exercise05] ラボ: MAC アドレステーブルと ARP の観察、デュプレックス

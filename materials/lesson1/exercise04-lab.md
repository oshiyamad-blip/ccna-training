# Exercise 4 ラボ手順書: IPv6 アドレッシング — 静的設定と SLAAC の構成

> 配置先: ドキュメント `02_ラボ手順書 > LESSON1 > Exercise04`
...
![Exercise 4 トポロジ図](../images/exercise04-topology.png)
...
PC1〜PC4 の設定が終わったら、ファイルを保存する: `File > Save As` → `exercise04_氏名.pkt`
...
2. `exercise04_氏名.pkt` を上書き保存する（`File > Save`、または `Ctrl + S`）
...
1. `exercise04_氏名.pkt` を Backlog のラボ課題に**添付**する

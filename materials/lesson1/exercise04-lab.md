# Exercise 4 ラボ手順書: VLSM によるアドレス設計とルーティング

> 配置先: ドキュメント `02_ラボ手順書 > LESSON1 > Exercise4`
...
Packet Tracer では、Exercise 3 で行ったロールオーバーケーブルによるコンソール接続を
...
設定後、`File > Save As` で `exercise04_氏名.pkt` として保存します。

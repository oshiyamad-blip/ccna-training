# Exercise 4 講義: IPv4 アドレッシングとサブネット化

> 配置先: ドキュメント `01_教材 > LESSON1 > Exercise4`
...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 3「Cisco IOS の基本操作とデバイス初期設定」 / Exercise 1「ネットワークの全体像と OSI / TCP-IP モデル」 の範囲から出題）。

**W1.** ユーザ EXEC モードから特権 EXEC モードへ、さらにグローバル設定モードへ
移行するコマンドを順に答えよ。

**W2.** データを送信するとき、トランスポート層・ネットワーク層・データリンク層で
それぞれ何と呼ばれる単位（PDU）になるかを、上位から順に答えよ。

<details><summary>解答</summary>

W1. `enable` → `configure terminal`
W2. セグメント（L4）→ パケット（L3）→ フレーム（L2）
...
`0xxxxxxx`（00000001〜01111110）の形になり、10 進数に直すと 1〜126 の範囲に収まります
（LESSON0 P2 で学んだ 2 進数⇔10 進数の変換と同じ考え方です）。先頭ビットは `0` で固定、
残り 7 ビットが変化します。すべて `0` の `00000000`（= 0）と、すべて `1` の
`01111111`（= 127）はそれぞれ予約されているため、クラスA の先頭オクテットは
1〜126 の範囲になります。

**127.0.0.0/8** は、先頭ビットのパターン（`0xxxxxxx`）の上ではクラスA の空間に属します。
ただし上で見たとおり、クラスA のアドレス範囲（1〜126）からは除外されており、
ホストへは割り当てず**ループバック**専用として予約されています。特に `127.0.0.1` は
「自分自身」を指すアドレスとして `localhost` の名前でもよく知られています。
...
**RFC1918** は、社内 LAN のようにインターネットへ直接経路広告（他のネットワークへ
「ここへの行き方」を知らせること）されない環境で、自由に使ってよいアドレス範囲を
定めた文書です。なお RFC とは Request for Comments の略で、インターネット技術の
標準・仕様を定めた文書群の呼び名です。CCNA では次の 3 つの範囲を暗記します。
...
IPv4 アドレスには、使える場所の違いで 2 種類あります。

- **私設アドレス** — 上の 3 範囲。社内 LAN や家庭内など**閉じた
  ネットワークの中だけ**で使うアドレス。世界中の LAN で同じ範囲を使い回してよい
- **グローバルアドレス** — インターネット上で**世界に 1 つだけ**割り当てられる公開
  アドレス。Web サーバなど、インターネットから直接届く必要がある機器が持つ

私設アドレスは、インターネット上ではルーティングされません。
そのため、私設アドレスを持つ端末がインターネットなどの外部と通信するには、
グローバルアドレスへの変換が必要です。この変換を担うのが **NAT（Network Address
Translation）** と、ポート番号を使って複数の端末で 1 つのグローバルアドレスを
共有する **PAT（Port Address Translation）** です。NAT/PAT の設定詳細は Exercise 13 で
扱いますが、ここでは「私設アドレスのままではインターネットに出られない」という前提を
押さえておいてください。

私設アドレスは、社内で重複さえしなければ自由に設計できます。そのため（1）限られた
グローバルアドレスの枯渇対策と、（2）柔軟なアドレス設計の両面で広く使われています。
...
> 💼 **実務では**: 保守の現場でこの重複が実際に牙を剥くのは、「自宅から VPN に
> 繋がらない」という一次切り分け依頼が来たときです。まず `ipconfig` や `show ip
> interface brief` などの確認コマンドで、自宅ルータ側のアドレス帯（多くが既定の
> `192.168.1.0/24`）と社内の払い出し帯を突き合わせます。重複の疑いが見えても、その場で
> アドレス設計を変更するような判断は行いません。切り分け結果を IPAM（IP アドレス
> 管理台帳）と照らして記録し、手順書に従って先輩や設計担当へエスカレーションします。
> こうした報告の精度が評価されると、次はその再設計案件に構築側として呼ばれる
> ことがあります。
...
この表は LAN でよく使う範囲の抜粋で、**プレフィックス長は /0 から /32 まで 1 ビット
刻みですべて存在します**。たとえば /23（255.255.254.0。/24 を 2 本束ねた大きさ）や、
機器 1 台をピンポイントで指す /32（255.255.255.255）も実際の設計・試験の両方に
登場します（/31 の特殊な使い方はこのあとの「/30 と /31」の節で触れます）。

サブネットマスクの各オクテットに現れる 10 進値は、`0`（そのオクテット全体がホスト部）と
`255`（全体がネットワーク部）を含めても、次の **9 種類しかありません**。

```
0, 128, 192, 224, 240, 248, 252, 254, 255
```

このうち、ネットワーク部とホスト部の境界にあたるオクテットに現れるのは
`128, 192, 224, 240, 248, 252, 254` の 7 種類です。この 7 つの並びをそのまま
暗記しておくと、サブネット計算が速くなります。
...
> **ここがこの Exercise の山場です。** ここから先のブロックサイズを使ったネットワーク・
> ブロードキャストアドレスの計算は、この Exercise の学習の中心であり、CCNA 試験でも
> 最頻出のテーマです。焦らず、1 問ずつ手を動かしながら進めてください。時間をかけて
> 構いません。
...
| /31 | 255.255.255.254 | 2（RFC3021） | ネットワーク／ブロードキャストの概念を持たず、2 アドレスともホストとして使う。さらにアドレス効率が良い |
...
> 💼 **実務では**: 保守現場で `/30` か `/31` かを気にするのは、主に構成台帳との
> 照合の場面です。`show ip interface brief` や `show running-config interface` で
> WAN リンクの状態を確認し、台帳や手順書に記載された値と一致しているかを照らします。
> マスクが想定と違っていても、客先で勝手にその場で直すことはしません。事象を記録して
> 先輩やお客様担当にエスカレーションするのが、客先常駐の基本ルールです。こうした
> 確認・報告を確実にこなせるようになると、次はループバックや P2P リンクのアドレス
> 計画そのものを任される構築案件へのステップアップが見えてきます。
...
例えば **100 台**を収容したい場合を考えます。2^7 − 2 = 126 ≥ 100 を満たすので、
...
![VLSM設計: 大きいブロックから連続して割り当てる](../images/exercise04-vlsm-block-allocation.png)
...
広告できます。詳細な集約の計算は、ルーティングプロトコルを学ぶ後続の Exercise で扱います。
...
2. 172.16.50.64（マスク /26、ブロックサイズ 64。境界は .0/.64/.128/.192 で
   `.90` は `.64`〜`.127` の範囲）
3. RFC1918 で定義された私設アドレスであり、インターネット上では
...
この Exercise のラボ課題「[Exercise04] ラボ: VLSM によるアドレス設計とルーティング」に
進み、ここまでに学んだサブネット計算・VLSM 設計を、実際のルータ・スイッチ構成の中で
実践してください。

...
![信頼ゾーンと DMZ](../images/day16-dmz-zones.png)
...
認証があります。認証局（CA、Certificate Authority）が発行する**公開鍵証明書
（Public Key Certificate）**を端末やユーザに割り当て、秘密鍵の所持を証明させることで
本人・端末を認証する方式です。
...
> 💼 **実務では**: 機器 1 台ずつローカルユーザを作るのは台数が増えると破綻するため、
> Cisco ISE などの TACACS+ サーバで管理者アカウントとコマンド認可を一元化し、
> SSH は踏み台（bastion/jump）ホスト経由に限定するのが定石です。新人が最も詰まるのは
> SSH 鍵生成の前提で、`ip domain-name` と `hostname` を先に設定していないと
> `crypto key generate rsa` が通らない点。また `transport input ssh` を入れ忘れて
> Telnet が空いたまま、というのが監査でよく指摘される定番の抜けです。
...
> 💼 **実務では**: `login block-for` のクワイエットモードに入ると原則すべての
> ログインが遮断されるため、リモート作業中の管理者自身が締め出される事故が
> 起きがちです。これを避けるため、実運用では `login quiet-mode access-class <ACL>`
> で管理サブネットからのアクセスだけは遮断対象から除外し、あわせてコンソール接続を
> 確保しておくのが定石です。変更作業（メンテナンスウィンドウ）中にわざと失敗を
> 繰り返してロックアウトされ、現地対応に呼び戻される、というのが新人のやりがちな
> ミスです。

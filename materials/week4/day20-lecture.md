...
![CCNA 200-301 ドメイン別配点比率](../images/day20-domain-score-weight.png)
...
>
> - **有効ホスト数** = 2^(ホストビット数) − 2
> - **マジックナンバー** = 256 − （マスクが掛かるオクテット〈Day 3 で学んだ、
...
- **BPDU Guard**: PortFast を設定したポートに BPDU（STP の制御フレーム）が
  届いた場合、そのポートを自動的に errdisable（Day 18 で学んだ、異常を検知
...
Day 12 で学んだとおり、OSPF のインターフェースコストは既定で次の式によって
...
Day 18 で学んだとおり、**IPsec** は VPN でよく使われる暗号化・認証のプロトコル
...
Day 19 で学んだとおり、**Ansible** は SSH ベースでエージェントレスに動作し、
...
この教材の学習を終えたら、総合演習ラボ「[Day20] ラボ: 総合演習 — 小規模企業ネットワークの

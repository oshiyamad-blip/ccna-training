# exercise02 .pktビルドシート

- **対象ラボ**: `materials/lesson1/exercise02-lab.md`（IOS の基本操作とデバイス初期設定 — ホスト名／enable secret／管理IP／SSHv2 有効化・Telnet 排除）
...
- **保存ファイル名**: `exercise02_start.pkt`
- **参照した図**: `materials/images/exercise02-topology.svg`（本文の結線・IP・機器と完全一致を確認済み）
...
開始ファイル（`exercise02_start.pkt`）では、下表の値をあらかじめ Desktop > IP Configuration に入力済みにして保存する（レベル A のため PC 側は完成済み）。
...
## 5. 完成コンフィグ（`exercise02_answer.pkt` 用・講師用）
...
- [ ] `exercise02_start.pkt` では R1・SW1・SW2 が初期状態（ホスト名未設定、`Router>` / `Switch>` プロンプト、IP・SSH設定なし）＝レベル A どおり本題が未設定で残っている
- [ ] `exercise02_start.pkt` では Admin-PC の RS232 ⟷ R1/SW1/SW2 の Console ポートに、コンソールケーブルが**一切接続されていない**（手順1で受講者自身が接続する学習内容のため）
- [ ] PC1 / PC2 / Admin-PC の IP Configuration が上記の値で入力済み
- [ ] 講師用 `exercise02_answer.pkt` を別途作成し、上記「完成コンフィグ」を R1・SW1・SW2 に投入した状態で、Admin-PC から `ping 192.168.1.1` / `ping 192.168.1.11` / `ping 192.168.1.12` が全て成功することを確認
- [ ] `exercise02_answer.pkt` で Admin-PC → SW1 への `ssh -l admin 192.168.1.11`（パスワード `adminpw`）が成功し、`SW1# show ip interface brief` が実行できることを確認
- [ ] `exercise02_answer.pkt` で Admin-PC → R1 への `telnet 192.168.1.1` が拒否される（`transport input ssh` の効果）ことを確認
- [ ] `exercise02_answer.pkt` の SW1 のみ `service password-encryption` が適用済み（R1・SW2 には適用しない、手順11の指示どおりの非対称性を再現）
- [ ] `.pkt` として保存後、一度閉じて再度開き、配線・状態（未設定/設定済み）が保たれていることを確認
- [ ] ファイル名が `exercise02_start.pkt`（開始用）/ `exercise02_answer.pkt`（完成見本）になっている

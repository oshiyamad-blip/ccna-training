# exercise09 .pktビルドシート

- **対象ラボ**: `materials/lesson2/exercise09-lab.md`（STP の動作観察とルートブリッジの変更、EtherChannel の構成）
- **作り込みレベル**: **A（配線済み・IP済み）** — 機器配置・全ケーブル・PC の IP 設定まで済ませ、
  ...
  受講者が行う**（`pkt-build-spec.md` の exercise09 行の指定どおり）。
- **保存ファイル名**: `exercise09_start.pkt`

> 参考画像: `materials/images/exercise09-topology.svg` を確認済み。結線・ポート番号・IP は
...
> `exercise09-topology.png` は本リポジトリの `images/` に存在するが、結線の正はラボ手順書の
...
- [ ] 保存前のテストとして、スイッチ初期状態（VLAN1 一括のフラットネットワーク）でも
      ...
      してから保存し、汚れのない状態で `exercise09_start.pkt` を残す
- [ ] `exercise09_start.pkt` として保存し、再度開いて上記の状態（配線・PC IP 済み、

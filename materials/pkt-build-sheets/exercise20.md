# exercise20 .pktビルドシート

- **対象ラボ**: `materials/lesson4/exercise20-lab.md`（総合演習 — 小規模企業ネットワークの構築）
- **作り込みレベル**: C（大規模・要注意）— `pkt-build-spec.md` の exercise20 行に従う
- **保存ファイル名**: `exercise20_start.pkt`

## レベル解釈の注意（このシート特有の判断）

`pkt-build-spec.md` の exercise20 行は「C」と表記されているが、特記事項には
...
見本であり、開始ファイル（`exercise20_start.pkt`）には含めない。`crypto key generate rsa` は
...
開始ファイル（`exercise20_start.pkt`）は IOS 設定が一切ないため、IP 到達性に関するチェックは
「物理層・PC/サーバ側の設定」のみが対象になる。IP 疎通・DHCP・HSRP 等の確認は完成ファイル
（`_answer.pkt`）側で行う。

### 開始ファイル（`exercise20_start.pkt`）のチェック
...
- [ ] `File > Save As` で `exercise20_start.pkt` として保存し、一度閉じて再度開いても
      上記の状態（IOS設定が空、PC/サーバ設定のみ投入済み）が保たれている

### 完成ファイル（`exercise20_answer.pkt`）のチェック（講師用見本の検収）

# exercise17 .pktビルドシート

- **対象ラボ**: `materials/lesson4/exercise17-lab.md`（拡張 ACL によるアクセス制御と VTY 管理制限）
- **作り込みレベル**: C（前提設定済み）— `pkt-build-spec.md` の exercise17 行に従う
- **保存ファイル名**: `exercise17_start.pkt`

## レベルCの解釈（このシート特有の判断）

`pkt-build-spec.md` は exercise17 を「VLAN間ルーティングやサーバ配置など前提を完成済みにし、
...
- **事前投入する（本題ではない）**: 手順1（VLAN・スイッチポート）、手順2
  ...
  手順7（SSH の基本設定 — 手順書内で明記のとおり「Exercise16 復習」であり ACL の本題ではないため）
...
> **4331 を使うのは意図的です。2911 に置き換えないでください。** 本編で唯一 ISR 4000 系を
> 使う回で、`Gi0/0/0` という 3 桁のインターフェース名に一度触れさせる狙いがあります
> （講義 第 4 章「インターフェース名の桁数が機種によって違う」で解説）。
> 他の Exercise はすべて 2911（`Gi0/0`）です。
...
見本であり、開始ファイル（`exercise17_start.pkt`）には含めない。
...
- [ ] `File > Save As` で `exercise17_start.pkt` として保存し、一度閉じて再度開いても

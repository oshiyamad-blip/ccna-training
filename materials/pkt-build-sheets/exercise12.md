# exercise12 .pktビルドシート

- **対象ラボ**: `materials/lesson3/exercise12-lab.md`（OSPF トラブルシューティングと HSRP による
  デフォルトゲートウェイ冗長化）
- **作り込みレベル**: **C（前提設定済み）** — `pkt-build-spec.md` の exercise12 行の指定どおり、
...
- **保存ファイル名**: `exercise12_start.pkt`

> `materials/images/exercise12-topology.svg` は本セッションの時点で存在しなかったため、
> 結線の確認は `exercise12-lab.md` の「完成トポロジ」節の IP アドレス表（本文 33〜44 行目）
...
Exercise12 は「意図的なミス入りの OSPF 設定」がラボの出発点そのものである。したがって
`exercise12_start.pkt` は、他の日のような「本題部分だけ空にする」のではなく、
...
- IP アドレス表（`exercise12-lab.md` 33〜44 行目）の「接続先」列と 1 対 1 で一致させている。
...
- [ ] PC1↔PC3・PC2↔PC3 の ping は**失敗**する（OSPF 未修復のため経路がない） ＝
      ...
      残っていること
- [ ] `exercise12_start.pkt` として保存し、再度開いて配線・IP・OSPF の障害入り設定が
      保持されていることを確認

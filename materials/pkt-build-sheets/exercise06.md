# exercise06 .pktビルドシート

- **対象ラボ**: `materials/lesson2/exercise06-lab.md`（VLAN の作成とブロードキャストドメインの分離検証）
- **作り込みレベル**: **A（配線済み・IP済み）** — 機器配置・全ケーブル・PC の IP 設定まで済ませ、SW1 は
  ホスト名すら設定していない初期状態（`Switch>` のまま）で保存する。この Exercise のテーマ（VLAN 10/20 の作成、
  アクセスポートへの割り当て）はすべて受講者が行う。
- **保存ファイル名**: `exercise06_start.pkt`

> 参考画像: `materials/images/exercise06-topology.svg` は現状リポジトリに存在しない
> （ラボ手順書が参照する `exercise06-topology.png` も未配置）。本シートは
> `exercise06-lab.md` の「IP アドレス表」および手順1の結線指示を正として作成している。
...
- [ ] 保存前の動作確認として PC1 の Command Prompt から `ping 192.168.10.12` を
      ...
      （または一度再配置）してから保存し、汚れのない状態で `exercise06_start.pkt` を残す
- [ ] `exercise06_start.pkt` として保存し、再度開いて上記の状態（配線・PC IP 済み、

...
- **対象ラボ**: `materials/lesson3/exercise11-lab.md`（静的ルートとフローティングスタティックの構成 —
  R1-R2-R3 直列トポロジ、IPv4/IPv6 静的ルート・デフォルトルート・フローティングスタティック）
- **作り込みレベル**: **A（配線済み・IP済み）** — `pkt-build-spec.md` の exercise11 行の指定どおり、
  「ルータ3台+PC配線済み・各インターフェースIP済み・PC IP済み。ルーティング設定は空
  （静的ルート投入が本題）」とする。汎用の A 定義（ルータ/スイッチは未設定＝ホスト名すら
  未設定の初期状態）ではなく、この exercise11 個別指定を優先する。具体的には、ラボ手順書の
...
exercise11 の本題であり、受講者が入力する部分として空のまま残す。
...
- [ ] `exercise11_start.pkt` として保存し、再度開いて配線・ホスト名・インターフェース

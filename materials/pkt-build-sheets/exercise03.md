...
- **対象ラボ**: `materials/lesson1/exercise03-lab.md`（IOS の基本操作とデバイス初期設定 — ホスト名／enable secret／管理IP／SSHv2 有効化・Telnet 排除）
- **作り込みレベル**: **A（配線済み・IP済み）** — `pkt-build-spec.md` の LESSON1 指定どおり。理由: ホスト名・パスワード・管理IP・SSH設定という「IOS 初期設定」そのものがこの Exercise の学習対象なので、R1/SW1/SW2 は工場出荷の初期状態のまま渡す。PC 側（IP設定・配線）は本題ではないため作り込み済みにする
...
|---|---|---|---|
| ルータ | Router **2911**（Gi0/0 を標準搭載。オンボードのみで手順書のインターフェース名と一致、追加モジュール不要） | 1 | R1 |
...
開始ファイル（`exercise03_start.pkt`）では、下表の値をあらかじめ Desktop > IP Configuration に入力済みにして保存する（レベル A のため PC 側は完成済み）。
...
- これはレベル A の定義（機器配置＋全ケーブル＋PC/サーバのIP設定済み。ルータ/スイッチは未設定）どおりであり、ホスト名・パスワード・管理IP・SSHv2 有効化という「IOS 初期設定」そのものがこの Exercise の学習対象であるため。
...
- [ ] `exercise03_start.pkt` では R1・SW1・SW2 が初期状態（ホスト名未設定、`Router>` / `Switch>` プロンプト、IP・SSH設定なし）＝レベル A どおり本題が未設定で残っている
- [ ] `exercise03_start.pkt` では Admin-PC の RS232 ⟷ R1/SW1/SW2 の Console ポートに、コンソールケーブルが**一切接続されていない**（手順1で受講者自身が接続する学習内容のため）
- [ ] PC1 / PC2 / Admin-PC の IP Configuration が上記の値で入力済み
- [ ] 講師用 `exercise03_answer.pkt` を別途作成し、上記「完成コンフィグ」を R1・SW1・SW2 に投入した状態で、Admin-PC から `ping 192.168.1.1` / `ping 192.168.1.11` / `ping 192.168.1.12` が全て成功することを確認
- [ ] `exercise03_answer.pkt` で Admin-PC → SW1 への `ssh -l admin 192.168.1.11`（パスワード `adminpw`）が成功し、`SW1# show ip interface brief` が実行できることを確認
- [ ] `exercise03_answer.pkt` で Admin-PC → R1 への `telnet 192.168.1.1` が拒否される（`transport input ssh` の効果）ことを確認
- [ ] `exercise03_answer.pkt` の SW1 のみ `service password-encryption` が適用済み（R1・SW2 には適用しない、手順11の指示どおりの非対称性を再現）

# exercise08 .pktビルドシート

- **対象ラボ**: `materials/lesson2/exercise08-lab.md`（VLAN 間ルーティング — Router-on-a-Stick と L3 スイッチ + SVI）
- **作り込みレベル**: **C（前提設定済み）** — `pkt-build-spec.md` の指定どおり、
  ...
  すべて受講者に残す**。
- **保存ファイル名**: `exercise08_start.pkt`

> 参考画像: `materials/images/exercise08-topology.svg`（Part1/Part2 双方の完成トポロジ）を
> 結線の確認に使用した。本シートの機器・ポート・IP は `exercise08-lab.md` の
...
Exercise8 は 1 本のラボの中でトポロジが **Part1（R1 + SW1 2960）→ Part2（R1 は放置、
SW1 を削除して MLS 3560 に置き換え）** と物理的に変化する。そのため `exercise08_start.pkt`
...
- Desktop > IP Configuration で Static を選択し、上記の IP / マスク / デフォルト
  ゲートウェイまで入力済みにする（レベル C は PC IP 済みが指定のため、Exercise1/Exercise3 の
  ように空欄で残す必要はない）。
...
- ホスト名は設定しない（プロンプトは `Switch>` のまま残る点は exercise06/exercise07 の
  ビルドシートと同様）。
...
### 5-2. 最終提出状態（Part2 完了後 = `exercise08_answer.pkt`）
...
- 手順8（発展）はオプション課題のため、`_answer.pkt` には含めるが `exercise08_start.pkt`
...
- [ ] `exercise08_start.pkt` として保存し、再度開いて配線・SW1 の VLAN/アクセスポート・

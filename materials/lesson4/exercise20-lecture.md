...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 13「OSPF 応用と
> FHRP」 / Exercise 17「ACL」 / Exercise 19「自動化とプログラマビリティ」 の範囲から
> 出題）。

**W1.** （Exercise 13）HSRP のプライオリティの既定値はいくつか。また `preempt` を
設定していない場合、後から復旧した高プライオリティのルータは Active の座を
自動的に奪い返せるか。

**W2.** （Exercise 17）/27 に対応するワイルドカードマスクは何か。また、標準 ACL と
拡張 ACL は、それぞれ経路上のどちら側（宛先の近く／送信元の近く）に適用する
のが原則か。

**W3.** （Exercise 19）REST API で新規リソースを作成する HTTP メソッドと、成功時の
...
いよいよ Exercise 20、研修の最終日です。本日は新しい技術を学ぶのではなく、これまで
...
LESSON1〜3 で受けた週次テスト（Exercise5・Exercise10・Exercise15）の誤答を振り返り、**ドメイン別
...
そしてこの教材を読み終えたら、**総合演習ラボ**で、VLAN・トランク・ルーティング（OSPF）・
...
>
> - **有効ホスト数** = 2^(ホストビット数) − 2
> - **マジックナンバー** = 256 − （マスクが掛かるオクテット〈Exercise 3 で学んだ、
>   IPv4 アドレスを 8 ビットずつ区切った単位〉の値）。マジックナンバーの倍数が
>   ネットワーク境界（アドレス）になる
...
- **BPDU Guard**: PortFast を設定したポートに BPDU（STP の制御フレーム）が
  届いた場合、そのポートを自動的に errdisable（Exercise 18 で学んだ、異常を検知
...
Exercise 12 で学んだとおり、OSPF のインターフェースコストは既定で次の式によって
...
> 手順書に沿って `show ip nat translations` を確認し、エントリが 0 件なら
> 変換対象トラフィックが届いていないと判断し、次に経路や ACL の設定を疑う、
> というように show コマンドを監視と切り分けの道具として使います。ここで
...
> 💼 **実務では**: 客先の保守現場でも port security 起因の errdisable は
> 日常的に発生し、社員の PC 入れ替えやドッキングステーション接続で
> maximum 超過が起きるたびに問い合わせが来ます。まず `show interfaces status`
> や `show port-security interface` でどのポートがどの理由で errdisable に
...
操作して設定・確認を行う**シミュレーション/シムレット**（1 つのシナリオ・
トポロジについて複数の設問が連続して出題される形式）があります。

# Exercise 16 講義: セキュリティの概念とデバイス保護

> 配置先: ドキュメント `01_教材 > LESSON4 > Exercise16`
...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 15「IPv6 アドレッシング」 / Exercise 13「NAT」 / Exercise 11「OSPFv2（シングルエリア）」 の範囲から出題）。

**W1.** IPv6 アドレスの省略ルールで、`::` は 1 つのアドレス中に何回まで使えるか。
またその理由は何か。

**W2.** PAT（NAT オーバーロード）が、1 つのグローバルアドレスで多数の内部ホストを
識別できるのはなぜか。

**W3.** OSPF のインターフェースコストの計算式を、既定の参照帯域幅とあわせて答えよ。

<details><summary>解答</summary>

W1. 1 回まで。2 回使うと省略された 0 ブロックの個数が一意に決まらなくなるため
W2. IP アドレスに加えて L4 のポート番号を変換テーブルに記録し、
戻りパケットの宛先ポートで内部ホストを一意に特定するため
W3. コスト = 参照帯域幅（既定 10^8 = 100Mbps）÷ インターフェース帯域幅
...
> この章では、以降のすべての土台となる用語と考え方を整理します。攻撃も対策も、
> ここで覚える言葉づかいの上で語られます。
...
![信頼ゾーンと DMZ](../images/exercise16-dmz-zones.png)

### 境界を守る専用機器 — ファイアウォールと IPS / IDS

Exercise 1 で名前だけ紹介したファイアウォールと IPS の役割を、ここで整理します。
いずれも、信頼ゾーンと非信頼ゾーンの**境界**（インターネットとの接続点や DMZ の
出入口）に配置される代表的な緩和策です。

- **ステートフルファイアウォール**: 通過した通信の状態（どの端末がどこへ接続を
  始めたか）を**状態表（ステートテーブル）**に記録し、内側から始めた通信に対する
  **戻り方向の応答を自動的に許可**します。1 パケットずつ独立に許可・拒否を判定する
  ACL（Exercise 17 で学びます）と異なり、「行きを許可した通信の帰りは通す」という
  判断を機器が自動で行える点が特徴です。
- **次世代ファイアウォール（NGFW、Next-Generation Firewall）**: ポート番号だけで
  なく、通信の中身から**どのアプリケーションの通信か**を識別して制御できる
  ファイアウォールです。同じ HTTPS（TCP 443）でも、業務システムへの通信は許可し
  動画サイトへの通信は遮断する、といった制御ができます。
- **IPS（Intrusion Prevention System、侵入防止システム）**: 既知の攻撃パターン
  （**シグネチャ**）と通信を照合し、一致した通信を**その場で自動的に遮断**します。
  通信経路上に直列（**インライン**）に配置されているため遮断が可能です。似た用語の
  **IDS（Intrusion Detection System、侵入検知システム）**は検知・通知のみを行い、
  遮断はしません。「Prevention = 防止（遮断する）、Detection = 検知（知らせるだけ）」
  と対で覚えましょう。

このあと、Exercise 17 ではルータ自身で通信を絞り込む ACL を、Exercise 18 では
アクセス層スイッチで足元を守る L2 セキュリティを学びます。境界の専用機器と合わせて、
多層防御のそれぞれの層を担う技術として位置づけてください。
...
> 第 1 章で「脅威が脆弱性をエクスプロイトで突く」という関係を押さえました。
> この章では、その「突き方」が実際にはどのような手口として現れるのかを見ていきます。
...
> 攻撃の手口を知ったら、ここからは守る側の話です。この章では、機器やサービスへの
> アクセスを「誰か・何を許すか・何をしたか」の 3 つで管理する仕組みを扱います。
...
RADIUS と TACACS+ は用途が似ているため混同しやすく、CCNA でも定番の比較ポイントです。
一度で覚えようとせず、表の項目を 1 つずつ見比べながら整理してください。
...
この 3 要素が基本ですが、補助的な要素として**場所**（Somewhere you are。接続元が
どの国・地域か、社内ネットワークからのアクセスかどうか、など）と**行動**
（Something you do。タイピングの癖、署名の仕方、など）を加えることもあります。
試験では知識・所持・生体の 3 要素が中心ですが、場所・行動を加えた **5 要素**として
整理されることもある、と補足的に押さえておけば十分です。
...
> 第 3 章が「誰にアクセスを許すか」の仕組みだったのに対し、この章では機器そのものを
> 固くするために、実際に入力する設定コマンドを扱います。
...
| コマンド | 保存方式 | 推奨度 |
|---|---|---|
| `enable secret` | ハッシュ化して保存（既定は MD5 = Type 5。より強い Type 8 = PBKDF2、Type 9 = scrypt も選べる） | 推奨 |
...
鍵を生成する前提として、ホスト名（`hostname`）が既定値（`Router` / `Switch`）以外に
変更されていること、そして `ip domain-name` でドメイン名が設定されていることが必要です。
この 2 つから鍵の名前が作られるためです。
...
`crypto key generate rsa` を実行すると、`How many bits in the modulus [512]:` という
鍵の長さ（モジュラス）を尋ねるプロンプトが表示されます。ここで **768 ビット以上**を
指定してください。**512 ビットのままでは SSH バージョン 2 を有効化できません**
（`ip ssh version 2` が受け付けられません）。実務では 1024 ビット以上を推奨します。

> **試験のポイント**: SSHv2 を有効にするには RSA 鍵のモジュラスが **768 ビット以上**
> 必要である、という数字が問われます。「必須は 768 以上、推奨は 1024 以上」と
> セットで覚えましょう。
...
> 行います。**Cisco ISE**（認証・認可を一元管理する Cisco の専用サーバ製品）や
> TACACS+ による一元管理の設計は構築チームの領域ですが、保守で切り分けと報告の
> 質を評価されると、そうした構築案件に関わる足がかりになります。
...
- `login block-for 60 attempts 3 within 30`: 30 秒以内に 3 回ログインに失敗すると、
  60 秒間リモートログイン（Telnet・SSH・HTTP）の試行をブロックします。
  ブルートフォース攻撃の緩和に有効です。このブロックが発動している状態を
  **クワイエットモード**と呼びます。
...
`login block-for` のクワイエットモードが発動している間は、既定では**管理者自身のものも
含めて、Telnet・SSH・HTTP によるリモートログイン試行がすべて拒否**されます
（コンソール接続は対象外で、コンソールからは引き続きログインできます）。
攻撃者だけでなく正規の運用担当者もリモートからはログインできなくなるため、
対応中に自分が締め出される事故が起こり得ます。
...
なお、VTY 回線に適用する通常の `access-class <ACL> in`（Exercise 17 で扱います）は、
...
この Exercise のラボ課題「[Exercise16] ラボ: デバイス堅牢化 — SSH 管理とログイン保護の実装」に進み、

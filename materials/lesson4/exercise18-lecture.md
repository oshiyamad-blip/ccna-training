# Exercise 18 講義: レイヤ 2 セキュリティと VPN 概要

> 配置先: ドキュメント `01_教材 > LESSON4 > Exercise18`
...
なお、本講義は Exercise 16「セキュリティの概念とデバイス保護」の続編にあたります。
Exercise 16 で学んだ概念 → 本講義前半の L2（アクセス層スイッチ）での実装 →
後半の VPN、という一連の流れとして読み進めてください。

---

## ウォームアップ（想起クイズ）

> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 17「ACL」 / Exercise 15「IPv6 アドレッシング」 / Exercise 13「NAT」 の範囲から出題）。

**W1.** 標準 ACL と拡張 ACL の判断材料の違いと、それぞれの推奨設置場所を答えよ。

**W2.** リンクローカルアドレスの範囲（プレフィックス）と、その特徴を答えよ。

**W3.** サーバを固定のグローバルアドレスで常時公開したいときに使う NAT の種類と、
そのコマンドの先頭部分を答えよ。

<details><summary>解答</summary>

W1. 標準 ACL は送信元 IP のみで判断し宛先の近くに置く／
拡張 ACL は送信元・宛先 IP・プロトコル・ポート番号で判断し送信元の近くに置く
W2. `fe80::/10`。同一リンク内でのみ有効で、IPv6 を有効にしたインターフェースに
必ず自動生成される
W3. 静的 NAT。`ip nat inside source static <内部IP> <グローバルIP>`
...
これまでの Exercise では ACL やパスワード管理など、主に L3（ネットワーク層）以上の
...
| **VLAN ホッピング** | トランクネゴシエーション（DTP）の悪用や二重タグ付けにより、本来到達できないはずの VLAN へフレームを送り込む（Exercise 7 で学んだトランクの堅牢化 — `switchport nonegotiate` による DTP の無効化とネイティブ VLAN の変更 — が対策） |
...
また、**未使用ポートは shutdown しておく**こと（Exercise 16）と、**未使用ポートを
どこにも接続されていない VLAN に割り当てておく**こと（Exercise 6 のブラックホール
VLAN）も、L2 セキュリティの基本的な堅牢化です。この Exercise の内容とセットで
押さえておきましょう。
...
（または `trunk`）に固定されている必要があります。Exercise 7 で学んだ DTP
...
> **試験のポイント**: アクセスポートに音声 VLAN（Exercise 6 で学んだ、IP 電話の音声を
...
**err-disabled** は、Exercise 9 で BPDU Guard のところで学んだのと同じ、エラーを
検知したスイッチがポートを強制的に無効化する状態のことです。なお本教材では、
**状態の名前は `err-disabled`**、**コマンドの名前は `errdisable recovery`**
と使い分けます（IOS のコマンド側にはハイフンが入りません）。
...
> アラートや「このポートだけ通信できない」というユーザ報告への一次対応です。
...
![DHCPスヌーピングのtrusted/untrustedメッセージ制御](../images/exercise18-dhcp-snooping-trust-flow.png)
...
> **ここがこの Exercise の山場です。** DHCP スヌーピングとの依存関係が絡む
> ところなので、時間をかけて構いません。
...
> この Exercise のラボはポートセキュリティと DHCP スヌーピングのみで実施し、
> DAI は概念の理解にとどめます。

## 5. WAN 接続の背景知識（読み物）

ここからは、拠点と拠点を結ぶ **WAN（Wide Area Network、広域ネットワーク）** の
話に移ります。次の節で学ぶ VPN の位置づけを理解するために、WAN 接続の歴史的な
背景をまず読み物として押さえておきましょう。この節の内容が CCNA 本試験で直接
問われることは少ないため、暗記は不要です。「なぜ現在は VPN が主流なのか」の
理由がつかめれば十分です。

拠点間を結ぶ WAN サービスは、伝統的に次の 3 つに分類されてきました。

| 分類 | 仕組み | 特徴 |
|---|---|---|
| **専用線（Leased Line）** | 2 拠点間を通信事業者の回線で**占有**して結ぶ | 帯域・品質が保証される代わりに非常に高価 |
| **回線交換（Circuit Switching）** | 通信するときだけ相手との間に回線をつなぐ（電話をかける仕組みと同じ） | **PSTN**（公衆電話網）や **ISDN** がこの方式。接続中は回線を占有する |
| **パケット交換（Packet Switching）** | データをパケットに分割し、網の設備を多数の利用者で共有する | 現在の主流。インターネットもこの方式 |

利用者側がネットワークに接続するための**アクセス回線**も世代交代してきました。
既設の電話線を流用して高速化した **DSL**（Digital Subscriber Line。ADSL など）が
普及したのち、より高速な光ファイバの **FTTH**（Fiber To The Home）へ移行が進み、
日本では現在、光回線が主流です。

かつての拠点間接続といえば高価な専用線や ISDN でしたが、現在は「**安価な
インターネット回線（FTTH など）+ VPN**」で専用線を代替する構成が主流です。
公衆網であるインターネットをそのまま使うと盗聴・改ざんのリスクがあるため、
暗号化されたトンネルで拠点間を安全に結ぶ技術が必要になります。それが、次の節から
学ぶ VPN です。

## 6. VPN の分類 — サイト間 VPN とリモートアクセス VPN
...
Client、旧称 AnyConnect など）をインストールし、**SSL/TLS**（Exercise 2 で学んだ
...
サイト間 VPN で多数の拠点をつなぐとき、まず 1 拠点（本社）を中心に据え、各支社が
本社とだけトンネルを張る **ハブ&スポーク（hub-and-spoke）** という構成をとります。
本社が「ハブ（中心）」、各支社が「スポーク（放射状に伸びる先）」です。この構成には
2 つの弱点があります。1 つは、支社どうしが直接通信したいときも通信が必ずハブ
（本社）を経由するため遠回りになり非効率になること。もう 1 つは、拠点を 1 つ増やす
たびにハブ側の設定を追加していく必要があり、拠点数が増えるほど管理が煩雑になる
ことです。

**DMVPN（Dynamic Multipoint VPN、動的マルチポイント VPN）** は、この
ハブ&スポークを拡張し、上の弱点を解消する技術です。ふだんは各スポークがハブと
だけつながっていますが、スポークどうしが通信する必要が生じた時点で、その 2 拠点の
間に**動的に直接トンネル**を張り、ハブを経由せずに通信できます。通信が終われば
その直接トンネルは不要になります。また、拠点を増減する際もハブ側の設定を拠点ごとに
書き換える必要がなく、拠点数の変化に柔軟に対応できます。

> **試験のポイント**: DMVPN は「拠点の増減に強く、スポークどうしが必要なときだけ
> ハブを経由せず動的に直接トンネルを張れる VPN」という位置づけで押さえれば十分です。
> DMVPN の具体的な設定手順は厳密には CCNP 寄りの範囲であり、CCNA では
> **何のための技術か**（ハブ&スポークの拡張・拠点増減への柔軟性・スポーク間の
> 動的な直接トンネル）を説明できるレベルが問われます。

### 使い分けの判断

- 拠点ネットワーク全体を常時接続したい → **サイト間 VPN**
- 不特定多数の個人ユーザが一時的に接続したい → **リモートアクセス VPN**
- 多数の拠点を効率よくつなぎ、拠点の増減やスポーク間の直接通信にも柔軟に
  対応したい → **DMVPN**（サイト間 VPN／ハブ&スポークの拡張）
...
|---|---|---|
| **ESP**（Encapsulating Security Payload） | 50 | 暗号化 + 完全性 + 認証（実務での主流） |
| **AH**（Authentication Header） | 51 | 完全性・認証のみ（**暗号化は行わない**） |

> **試験のポイント**: ESP と AH の役割の違い、特に **AH は暗号化を行わない**
> という点を問う問題が頻出です。覚え方の一例: 「**ESP と AH の違いは暗号化の
> 有無**。AH は完全性・認証はできるが暗号化はしない、番号は 51（ESP の 50 の
> 次）」。
...
5. サイト間 VPN とリモートアクセス VPN のうち、端末にクライアントソフトの
   インストールが必要なのはどちらか。
6. 拠点の増減に柔軟で、スポークどうしが必要なときだけハブを経由せず動的に
   直接トンネルを張れる VPN 技術を何というか。
...
5. リモートアクセス VPN
6. DMVPN（Dynamic Multipoint VPN。ハブ&スポーク型サイト間 VPN の拡張）

</details>

## 次のステップ

この Exercise のラボ課題「[Exercise18] ラボ: ポートセキュリティと DHCP スヌーピングによる

...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 5「ルーティングの
> 基礎と静的ルート」 / Exercise 14「IP サービス総合」 / Exercise 17「ACL」 の範囲から
> 出題）。

**W1.** （Exercise 5）静的ルートの既定の AD（Administrative Distance）はいくつか。
また、フローティングスタティックとして機能させるには、この AD をどのように
設定する必要があるか。

**W2.** （Exercise 14）DHCP リレーエージェントを有効にするコマンドは何か。また、

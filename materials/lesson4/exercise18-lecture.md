...
> 教材を見ずに、まず自力で思い出してください（分散学習: Exercise 11「ルーティングの
> 基礎と静的ルート」 / Exercise 15「IP サービス総合」 / Exercise 17「ACL」 の範囲から
> 出題）。

**W1.** （Exercise 11）静的ルートの既定の AD（Administrative Distance）はいくつか。
また、フローティングスタティックとして機能させるには、この AD をどのように
設定する必要があるか。

**W2.** （Exercise 15）DHCP リレーエージェントを有効にするコマンドは何か。また、
このコマンドはルータのどちら側（クライアント側／サーバ側）のインターフェースに
設定するか。

**W3.** （Exercise 17）標準 ACL と拡張 ACL は、それぞれインターフェースのどちら寄り
...
側（VLAN の SVI など）のインターフェースに設定する。
...
Client、旧称 AnyConnect など）をインストールし、**SSL/TLS**（Exercise 5 で学んだ

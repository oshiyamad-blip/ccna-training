# Exercise 17 ラボ手順書: 拡張 ACL によるアクセス制御と VTY 管理制限

> 配置先: ドキュメント `02_ラボ手順書 > LESSON4 > Exercise17`
...
![Exercise 17 トポロジ図](../images/exercise17-topology.png)

### IP アドレス表

> **`Gi0/0/0` と数字が 3 つあるのは誤記ではありません。** このラボのルータは
> ISR 4331 で、Exercise8・Exercise20 で使う ISR 2911（`Gi0/0`）とは
> インターフェース名の桁数が違います。構成そのものは Exercise8 で組んだ
> Router-on-a-Stick と同じです。理由は講義の第 4 章
> 「インターフェース名の桁数が機種によって違う」を参照してください。
...
Router-on-a-Stick は、1 本の物理インターフェースの中にサブインターフェース
（VLAN ごとの仮想的なインターフェース）を複数作り、ルータ 1 台だけで VLAN 間
ルーティングを行う構成です。Exercise 8 で学んだ構成を、ここで実際に組み立てます。
...
## 手順 7: SSH の有効化（Exercise16 復習）（15 分）
...
4. ファイルを保存します: `File > Save As` → `exercise17_氏名.pkt`
...
1. `exercise17_氏名.pkt` を Backlog のラボ課題に**添付**する
...
| 症状 | 確認すること |
|---|---|
| `interface GigabitEthernet0/0.10` が受け付けられない・存在しないと言われる | このラボのルータは ISR 4331 なので、正しくは `GigabitEthernet0/0/0.10`（数字が 3 つ）。Exercise8 の 2911（`Gi0/0`）と桁数が違う。`show ip interface brief` で実在する名前を確認する |

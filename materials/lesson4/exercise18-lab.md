# Exercise 18 ラボ手順書: ポートセキュリティと DHCP スヌーピングによる L2 セキュリティ強化

> 配置先: ドキュメント `02_ラボ手順書 > LESSON4 > Exercise18`
...
![Exercise 18 トポロジ図](../images/exercise18-topology.png)
...
全機器 VLAN 1（この Exercise では VLAN セキュリティではなく L2 セキュリティ機能そのものに
...
2. 次の機器を配置し、置いた直後にそれぞれ名前を付ける
   ...
   - Switch **2960** × 1 → `SW1`
   - PC × 3 → `PC0` / `PC1` / `PC2`
   - Server × 1 → `Rogue Server`（不正 DHCP サーバ役）

3. 接続ツール（稲妻アイコン）で、次のとおり結線する

   | 機器A・ポート | ケーブル | 機器B・ポート |
   |---|---|---|
   | R1 Gi0/0 | ストレート | SW1 Gi0/1 |
   | PC0 Fa0 | ストレート | SW1 Fa0/1 |
   | PC1 Fa0 | ストレート | SW1 Fa0/2 |
   | Rogue Server Fa0 | ストレート | SW1 Fa0/23 |

4. すべてのリンクが**緑**になるまで待つ
   （スイッチが絡む区間は STP の収束に 30 秒ほどかかります）

> **PC2 は配置するだけで、まだ結線しません。** あとの手順で、ポートセキュリティが
> 効いている Fa0/1 に差し替えて違反を起こすために使います。

---
## 手順 2: R1 の DHCP サーバ設定（15 分）
...
## 手順 3: 不正 DHCP サーバ（Rogue Server）の準備（10 分）
...
## 手順 4: SW1 のアクセスポート固定とポートセキュリティ設定（30 分）
...
1. PC0 を SW1 の Fa0/1 から取り外す（ワークスペース上のケーブルをクリックして
   選択し、Delete キーで削除する）
2. 代わりに PC2（別の MAC アドレスを持つ端末）を Fa0/1 に接続する（Exercise 1 で
...
## 手順 7: 対比実験 — trust を外すとどうなるか（20 分）
...
1. `exercise18_氏名.pkt` として保存し、Backlog のラボ課題に**添付**する
2. 手順 5〜6 の各コマンド出力（スクリーンショット可）と、観察レポートの
   回答を課題の**コメント**に貼る
3. 課題の状態を「処理済み」に変更する

...
> プロトコル」 / Exercise 14「NAT」 / Exercise 16「セキュリティの概念とデバイス保護」 の
> 範囲から出題）。

**W1.** （Exercise 10）無線 LAN で、有線の CSMA/CD に相当する衝突回避の方式の名称は
何か。また、2.4GHz 帯で相互に干渉しない 3 つのチャネル番号を答えよ。

**W2.** （Exercise 14）PAT の設定で、動的 NAT と区別して多対 1 変換を有効にするために
...
| QoS 分類 | Exercise 15 で学んだ QoS（優先制御）の対象トラフィックを ACL で分類する |
...
に記録できます（Syslog の仕組み自体は Exercise 15・Exercise 16 で学んだ内容と

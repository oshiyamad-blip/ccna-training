# Exercise 17 講義: ACL（アクセスコントロールリスト）

> 配置先: ドキュメント `01_教材 > LESSON4 > Exercise17`
...
**ACL（Access Control List / アクセスコントロールリスト）** とは、通信を
**許可（permit）** するか **拒否（deny）** するかを判定する仕組みです。
判定の条件に使うのは、L3（IP ヘッダ）/ L4（TCP・UDP ヘッダ）の情報です。
具体的には、送信元 IP アドレス・宛先 IP アドレス・プロトコル・ポート番号
などを指定します。ACL は 1 つ 1 つのパケットを個別に、それ以前の通信の状態を
記憶せずに判定します。そのため **ステートレス**なフィルタと呼ばれます
（状態を保持するファイアウォールとの違いです）。
...
| QoS 分類 | Exercise 14 で学んだ QoS（優先制御）の対象トラフィックを ACL で分類する |
...
> 上のように明示的な deny 行を 1 つも書かなかった場合に、末尾の暗黙の
> deny any が効いて残りすべてを破棄します。なお `deny ip any any log` の
> ように明示的な deny-all を自分で書いた場合は、そちらが先に一致するため
> 暗黙の deny には実質到達しません（最初の一致で評価が確定するため）。
> 明示的に書く目的は、`log` を付けてヒットカウンタや Syslog で末尾遮断の
> 動作を可視化することにあります。
...
> **ここがこの Exercise の山場です。** ワイルドカードマスクはサブネットマスクと
> 数字の意味が正反対になるため、最初は混乱して当然です。焦らず、表と計算式を
> 何度も見比べながら理解して構いません。
...
> 💼 **実務では**: ワイルドカードマスクの取り違え（サブネットマスクをそのまま
> 貼り付けてしまう等）は IOS がエラーを出さずに受理してしまうため、「設定は
> 入っているのに意図と違う範囲に一致している」という、保守現場でもっとも
> 気づきにくい部類の障害です。自分で ACL を書き換える機会は少なくても、
> `show access-lists` のヒットカウンタで想定した行に想定どおりの通信が当たって
> いるかを確認するのは、日々の監視・切り分けで欠かせません。数字が想定と
> 食い違っていても、客先で勝手に ACL を触って直すのではなく、手順書の切り分け
> 範囲を超えたら確認結果を添えて先輩やリーダーへエスカレーションするのが基本
> ルールです。
...
ワイルドカードマスクも技術的には作成できます。たとえば `0.0.0.254` は
最終オクテットが `11111110` なので、**最下位ビット以外を無視し（最下位
ビットだけを一致条件として）**、偶数ホストまたは奇数ホストをまとめて
指定できます。ただし実務・試験では、まず「/プレフィックス長からの
機械的な変換」を確実にできることが優先です。
...
Standard IP access list BLOCK-SALES
   ...
   20 deny any
Router(config-std-nacl)# 5 deny host 192.168.10.99
Router(config-std-nacl)# do show access-lists BLOCK-SALES
Standard IP access list BLOCK-SALES
   5 deny host 192.168.10.99
   10 permit 192.168.10.0 0.0.0.255
   20 deny any
```
...
上のように、既存の permit（seq 10）より**前**のシーケンス番号を指定して
限定的な deny を挿入すれば、既存の行を削除・再入力せずに順序を正しく
制御できます。逆にここで seq 15 のように permit(10) より後ろへ挿入すると、
192.168.10.99 は先に seq 10 の広い permit に一致してしまい（first match）、
挿入した deny host は一切評価されません。前章の「限定的な条件を上に、
広い条件を下に」という原則どおり、挿入位置は必ず対象の permit より
上のシーケンス番号にする必要があります。
...
### インターフェース名の桁数が機種によって違う

ACL は必ず「どのインターフェースに、どの向きで」適用します。そのため、
**インターフェース名を正確に読めること**が前提になります。ここで戸惑いやすいのが、
**機種によって名前の桁数が違う**ことです。

| 機種 | 例 | 数字の意味 |
|---|---|---|
| ISR 2911（Exercise8・Exercise20 のラボ） | `GigabitEthernet0/0` | スロット / ポート |
| ISR 4331（この Exercise のラボ） | `GigabitEthernet0/0/0` | スロット / サブスロット / ポート |

Exercise8 の VLAN 間ルーティングでは `Gi0/0.10` と書きましたが、この Exercise の
ラボでは同じ Router-on-a-Stick を `Gi0/0/0.10` と書きます。**やっていることは
まったく同じ**で、違うのはルータの機種だけです。

数字が 1 つ増えているのは、ISR 4000 シリーズが「スロットの中にさらに
サブスロット（拡張カードを挿す枠）がある」構造になっているためです。
古い 2911 は「スロット / ポート」の 2 段階、新しい 4331 は
「スロット / サブスロット / ポート」の 3 段階、という違いです。

サブインターフェースの番号（`.10` の部分）は VLAN ID に対応させる慣習で、
機種が変わっても意味は変わりません。

> 💼 **実務では**: 保守の現場で最初に確認するのは、**その機器が何という名前の
> インターフェースを持っているか**です。手順書に `Gi0/0` と書いてあっても、
> 実機が 4000 シリーズなら該当するのは `Gi0/0/0` かもしれません。思い込みで
> 打たず、必ず `show ip interface brief` で実在するインターフェース名を確認して
> から設定に入ります。名前を間違えたまま ACL を適用すると、意図した通信は
> 遮断されないのに別の通信が止まる、という最も厄介な障害になります。
...
![拡張ACLと標準ACLの適用位置の比較](../images/exercise17-acl-placement.png)

拡張 ACL を宛先近くに置くと、不要な通信が経路の大半を消費してから最後に
捨てられるだけで、「無駄」が生じます。一方、標準 ACL を送信元近くに置くと、
本来は許可したい別の宛先への通信まで送信元だけで判定され、巻き込まれて
しまいます。この 2 つは**性質の異なる失敗**である点を区別して覚えておきましょう。
...
> 💼 **実務では**: `access-class` の適用は保守運用の現場では自分の判断だけで
> 行わず、必ず作業手順書に沿って進めます。permit の書き忘れなどで全 SSH
> セッションが切れる remote lockout は、コンソールセッションの確保や適用順序
> といった「作業前確認・戻し手順」を手順書に定めておく理由そのものです。手順
> どおりに進めても接続が戻らない場合は、自力で復旧を試みず、切り分け結果を
> 添えてすぐに先輩やリーダーへエスカレーションします。こうした慎重な手順を
> 保守で徹底して信頼を積むことが、将来 VTY 制限のようなポリシー設計を任される
> 構築案件へのステップアップにつながります。
...
に記録できます（Syslog の仕組み自体は Exercise 14・Exercise 16 で学んだ内容と
...
この Exercise のラボ課題「[Exercise17] ラボ: 拡張 ACL によるアクセス制御と VTY 管理制限」に
進み、この Exercise で学んだワイルドカードマスク・拡張 ACL・標準 ACL による VTY 制限を

# Exercise 18 小テスト: レイヤ 2 セキュリティと VPN 概要

> 運用: 設問部分を小テスト課題の本文（またはドキュメント）に掲載。
> 「解答・解説」は講師用フォルダに保管し、**次の Exercise の開始時**に受講者へ公開する。
> ルール: 10 問 / 35 分 / 教材参照なし。解答はコメントに「Q1: A」形式で提出。
...
| Q10 | 例 | 「機密性（暗号化）・完全性（改ざん検知）・認証（相手確認）・アンチリプレイ（再送検知）の 4 つを提供する。ESP は暗号化・完全性・認証を提供し、AH は完全性・認証は提供するが暗号化（機密性）は行わない。両者の決定的な違いは暗号化（機密性）の有無である」等、4 サービスと ESP/AH の違い（AH に暗号化がない点）に触れていれば正解 |

**採点**: 1 問 10 点（100 点満点）、70 点未満（70% 未満）は次の Exercise の冒頭で再テスト。
Q10 は趣旨が合っていれば 10 点。

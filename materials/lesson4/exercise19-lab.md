# Exercise 19 ラボ手順書: JSON データの読解と REST 的な GET/POST リクエストの観察

> 配置先: ドキュメント `02_ラボ手順書 > LESSON4 > Exercise19`
...
![Exercise 19 トポロジ図](../images/exercise19-topology.png)
...
| SW1（管理用 SVI） | VLAN1 | 192.168.10.2 | 255.255.255.0 | 192.168.10.1 |
...
5. 全リンクの●が緑になったら、`File > Save As` で `exercise19_氏名.pkt` として保存する
...
続けて、SW1 の CLI（Config タブまたは端末）で管理用 SVI（VLAN1）に IP アドレスを
設定します。
...
- 設定後、PC3 から `ping 192.168.10.2` が通ることを確認する（手順 3 の JSON に
  記載された SW1 の `managementIp` と一致します）
...
5. `RT1` と `SW1` それぞれの `managementIp` の値を読み取り、手順 2 で設定した
   IP アドレスと一致することを確認する
...
1. Server-PT をクリック → [Services] タブ → **IoT** を選択し、
   「Registration Server」を `On` にする
2. 同じ画面の「Server Username」欄に `admin`、「Server Password」欄に
   `Cisco12345` を入力し、設定を保存する（この値が、下の手順 5 でログイン
   する際に必要な認証情報になる）
3. PC2 の [Desktop] → **Web Browser** を開き、URL 欄に `http://192.168.10.10`
   と入力してアクセスする。表示されたログイン画面（Username / Password
   入力欄）に**何も入力せず** [Login] をクリックする
4. アクセスが拒否され、ログイン画面に留まることを確認する。これが REST で
   いう **401 Unauthorized（未認証）** に概念的に対応することを理解する
   （PT 上に数値の「401」は表示されない）
5. 続けて、上の 2. で設定した `admin` / `Cisco12345` をログイン画面に入力して
   [Login] をクリックし、ログインに成功する（デバイス一覧画面が表示される）
   ことを確認する
6. ログイン後の画面で [Register a New IoT Device]（新規デバイス登録）を
   クリックし、任意の Device Type・Device Name を入力して [Add]（または
   [Register]）をクリックする。これが新規リソースの作成 = POST 相当の操作
   にあたる
7. 登録操作の後、デバイスの**登録一覧に新しいデバイスが追加されていること**
   を確認する。これが REST でいう **POST（Create）＝ 201 Created** に概念的に
   対応することを理解する（PT 上に数値の「201」は表示されない）
8. GET（情報取得）と POST 相当（新規作成）の違いを、実際の操作を通じて比較する

## 手順 6: レイヤ2/レイヤ3疎通の確認（20 分）
...
3. PC1 の Command Prompt で `arp -a` を実行し、Server の MAC アドレスが
   解決されていることを確認する
4. これらの結果から、手順 4・5 で行った HTTP（アプリケーション層）の通信が、
   L2（MAC 到達性）と L3（IP 到達性）という下位レイヤの上に成り立っていること
   を確認する（※これは講義で学んだ VXLAN の「アンダーレイ／オーバーレイ」
   そのものではなく、あくまで＜下位層が動いて初めて上位層が動く＞という
   階層構造の比喩である。VXLAN のようなトンネル技術・ファブリックはこの
   ラボには登場しない）
...
1. `exercise19_氏名.pkt` を Backlog のラボ課題に**添付**する
...
| 認証機能で 401 相当の挙動（ログイン要求画面）が再現できない | Server-PT の [Services] → [IoT] で Registration Server が `On` になっているか、Server Username/Password が設定されているか確認する |
| IoT のログイン画面で正しい認証情報を入れてもログインできない | 手順 5-2 で設定した Username/Password と、手順 5-5 で入力した値が一致しているか確認する（大文字・小文字も一致させる） |

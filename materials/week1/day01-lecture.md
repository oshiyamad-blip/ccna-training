...
> ※ 表の「L1〜L7」は通信の役割を 7 段階に分けた層の番号です（詳しくは 3 節）。
> 数字が小さいほど物理配線に近く、大きいほどアプリケーションに近いと考えてください。
...
表中の **MAC アドレス** は、通信用の部品（ネットワークカード）1 つひとつに割り当てられた
固有の識別番号です。Week0 P3・P5 で学んだ IP アドレス（ネット上の住所）とは別の番号で、
機器そのものを指す「製造時に割り振られた背番号」のようなものだと考えてください。
また表中の **IPS**（Intrusion Prevention System、侵入防止システム）は、既知の攻撃パターンを
...
> **コリジョンドメインとブロードキャストドメイン**（本日の学習目標には含まれない発展知識・
> 参考として押さえておくと後日の学習がスムーズです）: コリジョンドメインとは、複数の機器が
> 同時に信号を送るとデータ同士がぶつかる（衝突する）可能性がある範囲のことです。
> ブロードキャストドメインとは、宛先を指定しない同報通信（ブロードキャスト）が届く範囲の
> ことです。L2 スイッチはポートごとにコリジョンドメインを分割します（1 ポート = 1 コリジョン
> ドメイン）。ルータはブロードキャストドメインを分割します。ハブ（L1）は全ポートが 1 つの
> コリジョンドメインです。

...
本日はまず、通信の役割を階層に分けて整理する「参照モデル」から学びます。層の考え方を
先に押さえると、続いて登場する機器（ルータ・スイッチ・ハブ）を「どの層で仕事をするか」で
整理でき、理解が一気に楽になるからです。そのあと機器・トポロジ・カプセル化へと進み、
今日 1 日でネットワークの全体像をつかみます。
...
固有の識別番号です。Week0 P3・P5 で学んだ IP アドレス（ネット上の住所）とは別の番号で、
機器そのものを指す「製造時に割り振られた背番号」のようなものだと考えてください。
...
> チケットに残しながら進めることで、設定変更が必要そうだと分かった時点、あるいは
> 原因が特定の層に絞り込めない時点で先輩や上位チームへエスカレーションします。
...
ポート番号を今日すべて暗記する必要はありません。TCP / UDP の詳しい仕組みと
ウェルノウンポートの暗記は Day 5 で改めて取り組みます。
...
検知して通信を自動的に遮断する仕組みで、ファイアウォールと合わせて使われることが多い機能です。
...
中継するため、**ブロードキャストドメインは分割しません**。なお、ルーティング機能を
加えた **L3 スイッチ**は VLAN 間通信などに使われます（詳しくは後日学びます）。
...
現在の現場でハブがほぼ使われなくなったのは、①スイッチが十分安価になり衝突のない
効率的な転送が当たり前になった、②無関係な機器にも全通信が届くため盗聴されやすい、
③送信と受信を同時に行えない（半二重）ため性能が出ない、という理由からです。
...
遅延が予測しやすく、ノンブロッキング（どのサーバ同士が同時に通信しても、途中の回線が
渋滞して待たされにくい構造のこと）なのが特長です（※同一 Leaf 配下のサーバ同士は
Spine を経由しません）。

下図は Spine 2 台 × Leaf 3 台のフルメッシュの例です。どの Leaf 同士の間にも
同じ長さの経路があることを確認してください。

![Spine-Leaf構成](../images/day01-spineleaf.png)
...
![カプセル化と非カプセル化の流れ](../images/day01-encapsulation.png)
...
- L2 ヘッダの**送信元・宛先 MAC アドレスは、ともにルータを通過するたびに書き換わる**が、
  ...
  Translation、IP アドレスを変換する技術〉を除く。詳しくは後日学びます）
- L4 の PDU は TCP = **セグメント**、UDP = **データグラム**と呼び分ける
  （総称してセグメントと呼ぶこともある）

> 💼 **実務では**: この「各層でヘッダが付く」構造は、監視ログだけでは
> 切り分けが進まないときに Wireshark などのパケットキャプチャで実際に確認します。
> ただし客先常駐の保守現場でキャプチャを取るのは「勝手に」ではなく、作業手順書に
> 沿って対象区間・取得のタイミングをあらかじめ決め、作業前後の状態を記録した上で
> 行うのが基本です。新人がやりがちなのは、ルータを越えた先で採ったキャプチャの
> 宛先 MAC が「相手 PC ではなく直前のルータの MAC」になっているのを見て
> 「宛先が違う、壊れている」と誤解することです。MAC はホップごとに書き換わるのが

...
2. [Show All/None] で一旦すべて消し、[Edit Filters] → **ICMP**（IPv4 タブ内）にチェックし、
   **ARP** は一覧内の独立した項目として別途チェックする
   （ARP は IP アドレス→MAC アドレスを解決するプロトコルで、IPv4 の中には含まれていません）
3. 画面下部ツールバーの **Power Cycle Devices** ボタンをクリックし、PC0・PC1・スイッチを
   再起動する（手順 3 の疎通確認で PC0 に登録済みの ARP キャッシュをクリアし、
   Simulation モードで ARP から観察できるようにするため。リンクが緑に戻るまで待つ）
4. PC0 から `ping 192.168.1.20` を実行し、再生（▶）ボタンでコマ送りする
5. PC0 から出た封筒（パケット）をクリックし、次を観察して記録する
   - **OSI Model タブ**: 何層から何層までヘッダが付いているか
   - **Outbound PDU Details**: Ethernet ヘッダの Src/Dest MAC、IP ヘッダの Src/Dest IP
6. スイッチ通過時に封筒をクリックし、**スイッチが L2（MAC）だけを見て転送している**
   ことを確認する
7. 最初の通信で **ARP**（誰が 192.168.1.20 ですか？）が先に流れることを確認する

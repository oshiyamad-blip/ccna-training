...
IPv4 アドレスは 32 ビット（Week0 P2 で学んだ 2 進数 32 桁）で表現され、
...
| 範囲 | 由来クラス | 用途の目安 |
|---|---|---|
| `10.0.0.0/8` | クラス A（1 個） | 大規模組織全体 |
| `172.16.0.0/12` | クラス B 16 個分 | 中規模拠点 |
| `192.168.0.0/16` | クラス C 256 個分 | 小規模 LAN・家庭 |

> `/12` や `/16` はそれぞれ複数のクラス B・クラス C ネットワークをまとめた範囲であり、
> クラスの既定マスク（クラス B は `/16`、クラス C は `/24`）とは別物です。
...
![NAT の 4 用語（inside/outside × local/global）](../images/day14-nat-terminology.png)
...
条件リストです。ACL 自体は Day17 で本格的に扱うため、ここでは動的 NAT に

...
4. R2 で Srv1 方向と WAN 方向の疎通を確保する。**静的 NAT・動的 NAT で使う
   ...
   connected ネットワーク（Day11 で学んだ直接接続ルート）に含まれ、R1 の
   Proxy ARP（Day11 で学んだ、本来の宛先ではないルータが ARP 要求に自分の
   MAC アドレスで応答してしまう動作）により R2 は追加のルーティング設定なしで
   戻りパケットを転送できます（R1→R2 側の到達確認は次で行う）

...
- **課題**: [Backlog API](https://developer.nulab.com/docs/backlog/) で課題を一括作成
  できるため、Exercise01〜20 の Exercise 別課題（マイルストーン = LESSON、カテゴリー = ドメイン、
  期限日付き）をスクリプトで自動生成可能。
...
- **小テスト**: Backlog に自動採点機能はない。選択肢は:
  1. **課題ベースの小テスト（推奨）** — 設問を課題本文に記載し、解答をコメントで提出。
     解答・解説は次の Exercise の開始時に公開するドキュメントで提供。完了状況はボード / ガントで追跡。
...
> 注記: 以下は本編（Exercise01〜20 / 140 時間）についての調査当時の見積り。その後、
> IT リテラシーゼロの受講者を前提に LESSON0 プレコース（P1〜P5 / 35 時間）を新設したため、
> 既定運用の総ボリュームは計 175 時間となる（IT 経験者は LESSON0 を省略可）。
...
なら現実的な設定（独学より効率が良いため）。1 Exercise 7 時間 × 20 回で、6 ドメインを

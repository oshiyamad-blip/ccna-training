...
- **課題**: [Backlog API](https://developer.nulab.com/docs/backlog/) で課題を一括作成
  できるため、20 営業日分の Exercise 別課題（マイルストーン = LESSON、カテゴリー = ドメイン、
  期限日付き）をスクリプトで自動生成可能。
...
> IT リテラシーゼロの受講者を前提に LESSON0 プレコース（5 営業日 / 35 時間）を新設したため、
> 既定運用の総ボリュームは計 25 営業日 / 175 時間となる（IT 経験者は LESSON0 を省略可）。

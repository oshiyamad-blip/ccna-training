...
受講者から見える課題名・リンク・テストの配置が食い違います。

```bash
node scripts/check-consistency.mjs            # 不整合があれば終了コード 1
node scripts/check-consistency.mjs --verbose  # 合格した検査も表示
```

Exercise の並べ替え・追加・削除や番号の一括置換をしたときは、作業の最後に
まとめてではなく**各ステップの直後に**通すと、どの操作が壊したかが分かります。
